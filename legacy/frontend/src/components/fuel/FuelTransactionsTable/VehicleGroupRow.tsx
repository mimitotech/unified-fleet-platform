...
import { fuelTd, fuelStickyDateTdMuted, fuelStickyUnitTdMuted, fuelStickyDateTd, fuelStickyUnitTd } from './fuelTableCells';
import {
  ...
  return (
    <React.Fragment>
      <tr
        ...
        onClick={onToggle}
      >
        <td className={cn(fuelStickyDateTdMuted, isExpanded && 'bg-primary/10')}>
        ...
        <td className={cn(fuelStickyUnitTdMuted, isExpanded && 'bg-primary/10')}>
          <div className="fuel-cell-inline inline-flex items-center justify-center gap-1 max-w-[140px]">
            <Truck className="w-3.5 h-3.5 text-primary shrink-0" />
            <div className="min-w-0 text-left">
              <div className="font-semibold truncate" title={group.unitName}>{group.unitName}</div>
              {group.driverName && (
        ...
        group.transactions.map((t) => {
          ...
          return (
            ...
            >
              ...
              <td className={fuelTd}>
                <span className="fuel-cell-inline block max-w-[140px] text-xs text-muted-foreground line-clamp-2" title={t.location || undefined}>
                  {t.location || '—'}
                </span>
              </td>
