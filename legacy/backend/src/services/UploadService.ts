...
export class UploadService {
    ...
    if (!schemaReady) {
      schemaReady = (async () => {
        try {
          ...
          console.warn('[uploads] could not ensure tenant_files.content:', (e as Error).message);
        }
        try {
          const { rows } = await query<{ cnt: number }>(
            `SELECT COUNT(*) AS cnt FROM information_schema.COLUMNS
          ...
          console.warn('[uploads] could not ensure tenant_files.public_url:', (e as Error).message);
        }
      })();
    }
    await schemaReady;
  ...
  static async saveTenantFile(
    ...
    await query(
      `INSERT INTO tenant_files (id, tenant_id, file_type, file_name, mime_type, file_path, size_bytes, content, public_url)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
      [id, tenantId, fileType, fileName, mime, filePath, buffer.length, buffer, publicUrl]
    );
  ...
  static async findByPublicUrl(
    publicUrl: string
  ): Promise<{ content: Buffer; mimeType: string; filePath: string | null } | null> {
    await this.ensureSchema();
    ...
    const { rows } = await query<{
      ...
      `SELECT content, mime_type, file_path FROM tenant_files
       WHERE public_url = $1
          OR file_path LIKE $2
          OR file_path LIKE $3
  ...
  static rehydrateToDisk(filePath: string | null, content: Buffer, relUnderUploads?: string): void {
    const targets = [
      filePath,
