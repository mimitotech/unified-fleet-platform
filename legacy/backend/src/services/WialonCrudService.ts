...
export class WialonCrudService {
  private static async call<T>(
    ...
    return withWialonClient(credentials, (client) => client.request<T>(svc, params));
  }

  // —— Units ——
  static createUnit(
    credentials: WialonCredentialsInput,
    input: { creatorId: number; name: string; hwTypeId: number; dataFlags?: number }
  ) {
  ...
  static renameUnit(credentials: WialonCredentialsInput, itemId: number, name: string) {
    return this.call(credentials, 'item/update_name', { itemId, name });
  }

  static updateUnitPhone(credentials: WialonCredentialsInput, itemId: number, phoneNumber: string) {
  ...
  static upsertSensor(
    ...
    input: {
      ...
      unit?: string;
      description?: string;
      table?: Array<{ x: number; a: number; b: number }>;
    }
  ) {
  ...
  // —— Geofences ——
  static async defaultResourceId(credentials: WialonCredentialsInput): Promise<number> {
    ...
    return withWialonClient(credentials, async (client) => {
      ...
      if (!resources.length) throw new Error('No Wialon resource found');
      return resources[0].id;
    });
  }

  static updateGeofence(
    credentials: WialonCredentialsInput,
    input: {
      resourceId?: number;
      id?: number;
  ...
  static getGeofenceData(credentials: WialonCredentialsInput, resourceId: number, geofenceIds?: number[]) {
  ...
  static getDriverBindings(
    credentials: WialonCredentialsInput,
    input: { resourceId?: number; unitId?: number; driverId?: number; from: number; to: number }
  ) {
    return withWialonClient(credentials, async (client) => {
      const resourceId = input.resourceId ?? (await this.defaultResourceId(credentials));
  ...
  // —— Routes ——
  static createRoute(credentials: WialonCredentialsInput, creatorId: number, name: string) {
    return this.call(credentials, 'core/create_route', { creatorId, name, dataFlags: 1 });
  }

  static updateRouteCheckpoints(
    credentials: WialonCredentialsInput,
    routeId: number,
    checkpoints: Array<{ name: string; lat: number; lng: number; radius?: number }>
  ) {
  ...
  static loadLastMessages(credentials: WialonCredentialsInput, unitId: number, count = 10) {
  ...
  static exportReport(
    credentials: WialonCredentialsInput,
    format: number,
    options?: { pageOrientation?: string; pageSize?: string }
  ) {
