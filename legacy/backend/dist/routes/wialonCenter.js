...
async function resolveMotherId(raw) {
    if (raw)
        return raw;
    const id = await WialonMotherAccountService.getDefaultId();
    if (!id)
        throw new Error('No Wialon mother account configured');
    return id;
}
const centerLive = createWialonLiveHandlers(async (req) => {
    const motherId = await resolveMotherId(req.query.motherId || req.query.motherAccountId || undefined);
    const creds = await WialonMotherAccountService.loadCreds(motherId);
    const accountId = req.query.accountId;
    if (accountId)
        return { ...creds, accountId: String(accountId) };
    return creds;
});
...
router.post('/centers/wialon/mothers', async (req, res) => {
    try {
        const body = (req.body.credentials || req.body);
...
router.put('/centers/wialon', async (req, res) => {
    try {
        const body = (req.body.credentials || req.body);
...
router.get('/centers/wialon/accounts/:accountId', async (req, res) => {
    try {
        ...
        if (Number.isNaN(accountId))
            return error(res, 'Invalid account id');
        const motherId = await resolveMotherId(req.query.motherId || req.query.motherAccountId || undefined);
        const creds = await WialonMotherAccountService.loadCreds(motherId);
...
router.post('/centers/wialon/accounts/:accountId/test', async (req, res) => {
    try {
        ...
        if (Number.isNaN(accountId))
            return error(res, 'Invalid account id');
        const motherId = await resolveMotherId(req.body?.motherAccountId ? String(req.body.motherAccountId) : req.body?.motherId ? String(req.body.motherId) : undefined);
        await PlatformIntegrationService.assertAccountAvailable(accountId, req.body?.exceptTenantId);
        const creds = await WialonMotherAccountService.loadCreds(motherId);
