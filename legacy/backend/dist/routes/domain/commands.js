...
router.get('/history', requireTenant, async (req, res) => {
    const { rows } = await query(`SELECT * FROM command_logs WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT 100`, [req.tenantId]);
    return success(res, toCamelRows(rows));
});
router.post('/:assetId', requireTenant, requireCommandAccess, async (req, res) => {
    const { command, params = {} } = req.body;
    if (!command)
        return error(res, 'command required');
    ...
    if (!resolved)
        return error(res, 'This asset has no command-capable telematics source (Wialon or TrackSolid)', 400);
    const cmd = COMMAND_MAP[command] || command;
    const { rows: assetRow } = await query(`SELECT name FROM assets WHERE id = $1`, [req.params.assetId]);
    let status = 'sent';
    let response = null;
    try {
    ...
    const { rows: logRows } = await query(`INSERT INTO command_logs (tenant_id, asset_id, external_asset_id, asset_name, command, params, status, response, source_type, created_by)
    ...
    if (status === 'failed') {
        return error(res, response?.error || 'Command failed', 502);
    }
