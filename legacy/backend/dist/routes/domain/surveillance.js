...
import { fetchUpstream, getStreamByAccessToken, getStreamUpstream, resolveProxyToken, rewriteM3u8Playlist, } from '../../services/wialonStreamCache.js';
...
router.get('/units/:unitId', requireTenant, async (req, res) => {
    try {
...
router.get('/units/:unitId/files', requireTenant, async (req, res) => {
    try {
        const unitId = parseInt(String(req.params.unitId), 10);
        if (Number.isNaN(unitId))
            return error(res, 'Invalid unit id');
...
router.post('/clips/share', requireTenant, async (req, res) => {
    try {
        ...
        const clipRef = {
            ...
            storageType: req.body?.storageType != null ? parseInt(String(req.body.storageType), 10) : undefined,
...
router.get('/units/:unitId/files/stream', requireTenant, async (req, res) => {
    try {
        const unitId = parseInt(String(req.params.unitId), 10);
        if (Number.isNaN(unitId))
            return error(res, 'Invalid unit id');
        ...
        if (!path)
            return error(res, 'path query parameter is required');
        const storageType = parseInt(String(req.query.storageType || '2'), 10);
        if (storageType !== 1 && storageType !== 2)
            return error(res, 'storageType must be 1 or 2');
        const download = req.query.download === '1' || req.query.download === 'true';
        const creds = await loadTenantWialonCreds(req.tenantId);
        const file = await WialonVideoService.readStorageFile(creds, unitId, storageType, path);
...
router.patch('/units/:unitId/cameras', requireTenant, requireCommandAccess, async (req, res) => {
    try {
        const unitId = parseInt(String(req.params.unitId), 10);
        if (Number.isNaN(unitId))
            return error(res, 'Invalid unit id');
        ...
        if (!Array.isArray(cameras) || !cameras.length)
            return error(res, 'cameras array is required');
        const creds = await loadTenantWialonCreds(req.tenantId);
        await WialonVideoService.updateVideoSettings(creds, req.tenantId, unitId, cameras.map((c) => ({
...
router.get('/units/:unitId/messages/:messageId/file', requireTenant, async (req, res) => {
    try {
        const unitId = parseInt(String(req.params.unitId), 10);
        const messageId = parseInt(String(req.params.messageId), 10);
        if (Number.isNaN(unitId) || Number.isNaN(messageId))
...
router.post('/units/:unitId/cameras/:channel/live/start', requireTenant, async (req, res) => {
    try {
        const unitId = parseInt(String(req.params.unitId), 10);
        const channel = parseInt(String(req.params.channel), 10);
        if (Number.isNaN(unitId) || Number.isNaN(channel))
            return error(res, 'Invalid unit or channel');
        const creds = await loadTenantWialonCreds(req.tenantId);
        const result = await WialonVideoStreamService.startLiveStream(req.tenantId, creds, unitId, channel);
...
function resolveLiveStreamEntry(req, unitId, channel) {
...
router.get('/units/:unitId/cameras/:channel/live/playlist.m3u8', requireTenant, async (req, res) => {
    try {
        const unitId = parseInt(String(req.params.unitId), 10);
        const channel = parseInt(String(req.params.channel), 10);
        if (Number.isNaN(unitId) || Number.isNaN(channel))
        ...
        if (!entry)
            return error(res, 'Live stream not started — click Go Live first');
        const upstream = await fetchUpstream(entry.upstreamUrl);
        const isPlaylist = /\.m3u8/i.test(entry.upstreamUrl) ||
...
router.get('/units/:unitId/cameras/:channel/live/stream', requireTenant, async (req, res) => {
    try {
        const unitId = parseInt(String(req.params.unitId), 10);
        const channel = parseInt(String(req.params.channel), 10);
        if (Number.isNaN(unitId) || Number.isNaN(channel))
...
router.get('/proxy/segment/:streamToken/:segToken', requireTenant, async (req, res) => {
    try {
        const streamToken = String(req.params.streamToken);
        const entry = getStreamByAccessToken(streamToken);
        if (!entry)
            return error(res, 'Live stream session expired — click Go Live again');
        const url = resolveProxyToken(String(req.params.segToken));
        if (!url)
            return error(res, 'Stream segment expired — restart live view');
        const range = req.headers.range ? String(req.headers.range) : undefined;
...
router.get('/embed-session', requireTenant, async (req, res) => {
    try {
        ...
        const creds = await loadTenantWialonCreds(req.tenantId);
        const session = await WialonVideoService.createEmbedSession(creds, unitId && !Number.isNaN(unitId) ? unitId : undefined, channel && !Number.isNaN(channel) ? channel : undefined);
...
router.post('/units/:unitId/commands', requireTenant, requireCommandAccess, async (req, res) => {
    try {
        const unitId = parseInt(String(req.params.unitId), 10);
...
router.get('/violations', requireTenant, async (req, res) => {
    ...
    const { rows: eco } = await query(`SELECT id, unit_id, unit_name, violation_type as type, severity, occurred_at, driver_name, 'eco' as source
     FROM eco_driving_violations WHERE tenant_id = $1 ORDER BY occurred_at DESC LIMIT $2`, [req.tenantId, limit]);
    const { rows: alerts } = await query(`SELECT a.id, a.title, a.type, a.severity, a.occurred_at, a.video_url, a.source_type as source,
     ...
     ORDER BY a.occurred_at DESC LIMIT $2`, [req.tenantId, limit]);
    const combined = [
        ...
        ...toCamelRows(alerts).map((r) => ({
            ...r,
            category: r.videoUrl ? 'video' : 'alert',
        })),
    ];
    ...
    const filtered = unitIdFilter != null && !Number.isNaN(unitIdFilter)
