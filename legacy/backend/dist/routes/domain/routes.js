...
router.get('/stats', requireTenant, mod, async (req, res) => {
    const { rows } = await query(`SELECT
     ...
     FROM fleet_routes WHERE tenant_id = $1 AND deleted_at IS NULL`, [req.tenantId]);
    return success(res, toCamelRows(rows)[0]);
});
router.get('/trips', requireTenant, mod, async (req, res) => {
...
router.post('/', requireTenant, mod, requireWriteAccess, async (req, res) => {
    const { name, status, assetId, assetName, assetPlate, driverId, driverName, startTime, distance, waypoints, eta, color, estimatedDuration, notes, } = req.body;
    if (!name)
        return error(res, 'name required');
    const { rows } = await query(`INSERT INTO fleet_routes (tenant_id, name, status, asset_id, asset_name, asset_plate, driver_id, driver_name,
...
router.patch('/:id', requireTenant, mod, requireWriteAccess, async (req, res) => {
    ...
    const { rows } = await query(`UPDATE fleet_routes SET
     ...
     RETURNING *`, [req.params.id, req.tenantId, status, endTime, actualStartTime, actualDuration, fuelUsage, notes]);
    if (!rows[0])
