...
export function createWialonCrudHandlers(loadCreds) {
    return {
        createUnit: async (req, res) => {
            const { creatorId, name, hwTypeId } = req.body;
            if (!creatorId || !name || !hwTypeId)
                return error(res, 'creatorId, name, hwTypeId required');
            try {
        ...
        patchUnit: async (req, res) => {
            ...
            try {
                ...
                const results = {};
                if (body.name)
                    results.name = await WialonCrudService.renameUnit(creds, unitId, String(body.name));
                if (body.phone)
                    results.phone = await WialonCrudService.updateUnitPhone(creds, unitId, String(body.phone));
                if (body.deviceTypeId && body.uniqueId) {
                    results.device = await WialonCrudService.updateDeviceType(creds, unitId, Number(body.deviceTypeId), String(body.uniqueId));
                }
        ...
        upsertSensor: async (req, res) => {
            const unitId = parseId(req.params.unitId);
            if (!unitId)
                return error(res, 'Invalid unit id');
            const { callMode, name, type, param, unit, description, table, id } = req.body;
            if (!callMode || !name || !type || !param)
                return error(res, 'callMode, name, type, param required');
            try {
                const creds = await loadCreds(req);
        ...
        patchGeofence: async (req, res) => {
            const { callMode, name, type, points, center, radius, resourceId, id, color, description } = req.body;
            if (!callMode || !name)
                return error(res, 'callMode and name required');
            try {
                const creds = await loadCreds(req);
        ...
        upsertDriver: async (req, res) => {
            const { callMode, name, code, phone, description, resourceId, id } = req.body;
            if (!callMode || !name)
                return error(res, 'callMode and name required');
            try {
                const creds = await loadCreds(req);
        ...
        bindDriver: async (req, res) => {
            const { unitId, driverId, assign, resourceId, time } = req.body;
            if (!unitId || !driverId)
                return error(res, 'unitId and driverId required');
            try {
                const creds = await loadCreds(req);
        ...
        upsertNotification: async (req, res) => {
            const { callMode, name, text, unitIds, trigger, actions, resourceId, id } = req.body;
            if (!callMode || !name || !trigger)
                return error(res, 'callMode, name, trigger required');
            try {
                const creds = await loadCreds(req);
        ...
        createRoute: async (req, res) => {
            ...
            if (!creatorId || !name)
                return error(res, 'creatorId and name required');
            try {
                const creds = await loadCreds(req);
        ...
        loadMessages: async (req, res) => {
            ...
            try {
                const creds = await loadCreds(req);
                const result = await WialonCrudService.loadMessages(creds, unitId, Math.floor(from / 1000), Math.floor(to / 1000), count);
                return success(res, { unitId, result });
            }
            catch (e) {
                return error(res, e.message);
        ...
        createTrackLayer: async (req, res) => {
            const unitId = parseId(req.params.unitId);
            if (!unitId)
                return error(res, 'Invalid unit id');
            const { from, to, layerName, trackColor, trackWidth, arrows } = req.body;
            if (!from || !to)
                return error(res, 'from and to required (unix ms)');
            try {
                const creds = await loadCreds(req);
