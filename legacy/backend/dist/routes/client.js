...
import { fleetUnitToAsset, fleetUnitToStatusItem, fleetToSnapshotResponse, } from '../services/wialonFleetMapper.js';
...
async function loadCachedLiveFleet(tenantId) {
    if (!(await WialonFleetService.isLiveAvailable(tenantId)))
        return null;
    try {
...
router.get('/tenant', requireTenant, async (req, res) => {
    ...
    const t = rows[0];
    const assetPath = (value) => normalizeUploadPath(value == null ? null : String(value));
    return success(res, {
        id: t.id,
        name: t.name,
        slug: t.slug,
...
router.get('/integrations/status', requireTenant, async (req, res) => {
    const { rows } = await query(`SELECT source_type, is_active, last_sync_at, last_error, connection_verified_at,
            wialon_account_name, wialon_resource_id, wialon_operate_as, wialon_session_meta, preview_asset_count
     FROM data_sources WHERE tenant_id = $1`, [req.tenantId]);
...
router.get('/modules', requireTenant, async (req, res) => {
    const { rows: connectedSources } = await query(`SELECT source_type FROM data_sources WHERE tenant_id = $1 AND is_active = true AND connection_verified_at IS NOT NULL`, [req.tenantId]);
    const connected = new Set(connectedSources.map((s) => s.source_type));
    const { rows } = await query(`SELECT md.\`key\` as moduleKey, md.label, md.icon, md.sources, md.sort_order as sortOrder,
     ...
     ORDER BY md.sort_order`, [req.tenantId]);
    ...
    if (req.user?.id && role !== 'platform_admin' && role !== 'super_admin' && role !== 'tenant_admin') {
        const { rows: userMods } = await query(`SELECT module_key, is_enabled FROM user_modules WHERE user_id = $1`, [req.user.id]);
    ...
    const result = filtered.map((m) => {
        ...
        const integrationReady = required.length === 0 || required.some((s) => connected.has(s));
        return { ...m, integrationReady };
    });
    return success(res, result);
});
// User preferences
router.get('/preferences', requireTenant, async (req, res) => {
    const { rows } = await query(`SELECT * FROM user_preferences WHERE user_id = $1`, [req.user.id]);
    if (rows[0])
        return success(res, toCamelCase(rows[0]));
    return success(res, {
...
router.put('/preferences', requireTenant, async (req, res) => {
    const b = req.body;
    const { rows } = await query(`INSERT INTO user_preferences (user_id, language, timezone, date_format, time_format, unit_system,
    ...
    return success(res, rows[0] ? toCamelCase(rows[0]) : null);
});
...
router.get('/users', requireTenant, async (req, res) => {
    ...
    const { rows } = await query(`SELECT id, email, full_name, role, is_active, last_login_at, created_at
     FROM users WHERE tenant_id = $1 ORDER BY full_name`, [req.tenantId]);
    return success(res, rows);
});
router.post('/users', requireTenant, async (req, res) => {
    if (!isTenantAdmin(req))
        return error(res, 'Forbidden', 403);
    const { email, password, fullName, role } = req.body;
    if (!email || !/\S+@\S+\.\S+/.test(email))
        return error(res, 'A valid email is required');
    const userRole = role || 'viewer';
    if (!isValidTenantRole(userRole))
        return error(res, 'Invalid role');
    const temporaryPassword = password || crypto.randomBytes(8).toString('hex');
    const hash = await bcrypt.hash(temporaryPassword, 10);
    try {
        const { rows } = await query(`INSERT INTO users (tenant_id, email, password_hash, full_name, role, force_password_change)
       VALUES ($1, $2, $3, $4, $5, true)
       RETURNING id, email, full_name, role, is_active, created_at`, [req.tenantId, email.toLowerCase().trim(), hash, fullName || email, userRole]);
...
router.patch('/users/:userId', requireTenant, async (req, res) => {
    if (!isTenantAdmin(req))
        return error(res, 'Forbidden', 403);
    ...
    const { rows } = await query(`UPDATE users SET full_name = COALESCE($3, full_name), role = COALESCE($4, role),
                      is_active = COALESCE($5, is_active), updated_at = NOW()
     WHERE id = $2 AND tenant_id = $1
     RETURNING id, email, full_name, role, is_active`, [req.tenantId, userId, fullName, role, isActive]);
    if (!rows[0])
...
router.delete('/users/:userId', requireTenant, async (req, res) => {
    if (!isTenantAdmin(req))
        return error(res, 'Forbidden', 403);
    const userId = String(req.params.userId);
    if (userId === req.user?.id)
        return error(res, 'You cannot remove your own account');
    const { rows } = await query(`UPDATE users SET is_active = false, updated_at = NOW()
     WHERE id = $2 AND tenant_id = $1 RETURNING id`, [req.tenantId, userId]);
    if (!rows[0])
        return error(res, 'User not found', 404);
    await AuditService.log({
        tenantId: req.tenantId,
...
router.post('/users/:userId/reset-password', requireTenant, async (req, res) => {
    if (!isTenantAdmin(req))
        return error(res, 'Forbidden', 403);
    const userId = String(req.params.userId);
    if (userId === req.user?.id) {
    ...
    const temporaryPassword = req.body?.password || crypto.randomBytes(8).toString('hex');
    const hash = await bcrypt.hash(temporaryPassword, 10);
    const { rows } = await query(`UPDATE users SET password_hash = $3, force_password_change = true, updated_at = NOW()
     WHERE id = $2 AND tenant_id = $1 RETURNING id, email`, [req.tenantId, userId, hash]);
    if (!rows[0])
        return error(res, 'User not found', 404);
    await AuditService.log({
        tenantId: req.tenantId,
...
// Dashboard KPIs
router.get('/dashboard/kpis', requireTenant, async (req, res) => {
...
router.get('/fleet/snapshot', requireTenant, async (req, res) => {
...
// Unified assets
router.get('/assets', requireTenant, async (req, res) => {
    const fleet = await loadCachedLiveFleet(req.tenantId);
    if (fleet) {
...
router.get('/assets/:id/status', requireTenant, async (req, res) => {
...
router.get('/assets/statuses', requireTenant, async (req, res) => {
    const fleet = await loadCachedLiveFleet(req.tenantId);
    if (fleet) {
...
router.post('/alerts/acknowledge-bulk', requireTenant, async (req, res) => {
...
router.post('/alerts/sync', requireTenant, async (req, res) => {
...
router.post('/alerts/:id/acknowledge', requireTenant, async (req, res) => {
    const orch = new AlertOrchestrator(req.tenantId);
