...
import { authMiddleware, requireAdminAccess, requireSuperAdmin } from '../middleware/auth.js';
import { requireTenantAccess } from '../middleware/tenantAccess.js';
import { isSuperAdmin } from '../utils/systemRoles.js';
...
import { assertCanManageClientUser, filterClientUserIdsForAdmin, getUserModules, isValidTenantRole, } from '../utils/userAccess.js';
...
router.put('/system/settings/:key', async (req, res) => {
    ...
    if (!value)
        return error(res, 'value required');
    await query(`INSERT INTO system_settings (\`key\`, value, updated_at) VALUES ($1, $2, NOW())
     ON CONFLICT (\`key\`) DO UPDATE SET value = $2, updated_at = NOW()`, [req.params.key, JSON.stringify(value)]);
...
router.post('/login-slides', async (req, res) => {
    try {
        const { LoginSlideService } = await import('../services/LoginSlideService.js');
...
router.patch('/login-slides/:id', async (req, res) => {
    try {
        const { LoginSlideService } = await import('../services/LoginSlideService.js');
        ...
        return error(res, e.message, /not found/i.test(e.message) ? 404 : 400);
    }
});
router.delete('/login-slides/:id', async (req, res) => {
    try {
        const { LoginSlideService } = await import('../services/LoginSlideService.js');
        const ok = await LoginSlideService.remove(String(req.params.id));
        if (!ok)
            return error(res, 'Slide not found', 404);
        await AuditService.log({
            userId: req.user?.id,
            userEmail: req.user?.email,
...
router.post('/login-trust-logos', async (req, res) => {
    try {
        const { LoginTrustLogoService } = await import('../services/LoginTrustLogoService.js');
...
router.patch('/login-trust-logos/:id', async (req, res) => {
    try {
        const { LoginTrustLogoService } = await import('../services/LoginTrustLogoService.js');
...
router.delete('/login-trust-logos/:id', async (req, res) => {
    try {
        const { LoginTrustLogoService } = await import('../services/LoginTrustLogoService.js');
        const ok = await LoginTrustLogoService.remove(String(req.params.id));
        if (!ok)
            return error(res, 'Logo not found', 404);
        await AuditService.log({
            userId: req.user?.id,
            userEmail: req.user?.email,
...
router.patch('/marketplace/:key', async (req, res) => {
    ...
    const { rows } = await query(`UPDATE marketplace_integrations SET is_enabled_globally = COALESCE($2, is_enabled_globally)
     WHERE \`key\` = $1 RETURNING *`, [req.params.key, isEnabledGlobally]);
    if (!rows[0])
        return error(res, 'Integration not found', 404);
...
router.get('/users', async (req, res) => {
    ...
    const { rows } = await query(`SELECT u.id, u.email, u.full_name, u.role, u.is_active, u.last_login_at, u.created_at,
            u.tenant_id, t.name as tenant_name, t.slug as tenant_slug
     FROM users u LEFT JOIN tenants t ON t.id = u.tenant_id
     ${where} ORDER BY u.created_at DESC LIMIT 200`, params);
    return success(res, rows);
});
...
router.patch('/users/:id', async (req, res) => {
    try {
        ...
        const { rows } = await query(`UPDATE users SET
         ...
         updated_at = NOW()
       WHERE id = $1 RETURNING id, email, full_name, role, is_active, tenant_id`, [userId, isActive, role, fullName]);
        if (!rows[0])
            return error(res, 'User not found', 404);
        if (Array.isArray(modules)) {
            await query(`DELETE FROM user_modules WHERE user_id = $1`, [userId]);
            for (const mod of modules) {
                await query(`INSERT INTO user_modules (user_id, module_key, is_enabled) VALUES ($1, $2, true)`, [userId, mod]);
            }
        }
        await AuditService.log({
            userId: req.user?.id,
...
router.post('/users/:id/reset-password', async (req, res) => {
    try {
        ...
        await query(`UPDATE users SET password_hash = $2, force_password_change = true, updated_at = NOW() WHERE id = $1`, [userId, hash]);
        await AuditService.log({
            userId: req.user?.id,
            userEmail: req.user?.email,
...
router.post('/users/bulk', async (req, res) => {
    ...
    if (!Array.isArray(userIds) || !userIds.length)
        return error(res, 'userIds required');
    const allowedIds = await filterClientUserIdsForAdmin(req, userIds);
    if (!allowedIds.length)
        return error(res, 'No permitted users in selection', 403);
    if (action === 'activate') {
...
router.post('/system-users', requireSuperAdmin, async (req, res) => {
    const { email, password, fullName, role } = req.body;
    if (!email || !password)
        return error(res, 'email and password required');
    const staffRole = role === 'super_admin' ? 'super_admin' : 'platform_admin';
    const hash = await bcrypt.hash(password, 10);
    const { rows } = await query(`INSERT INTO users (tenant_id, email, password_hash, full_name, role)
     VALUES (NULL, $1, $2, $3, $4)
     RETURNING id, email, full_name, role, is_active, created_at`, [String(email).toLowerCase().trim(), hash, fullName || email, staffRole]);
    await AuditService.log({
        userId: req.user?.id,
        userEmail: req.user?.email,
...
router.patch('/system-users/:id', requireSuperAdmin, async (req, res) => {
    ...
    const { rows } = await query(`UPDATE users SET
     ...
     RETURNING id, email, full_name, role, is_active`, [req.params.id, isActive, staffRole, fullName]);
    if (!rows[0])
        return error(res, 'System user not found', 404);
    return success(res, rows[0]);
});
router.post('/system-users/:id/reset-password', requireSuperAdmin, async (req, res) => {
    const newPassword = req.body.password || crypto.randomBytes(8).toString('hex');
    const hash = await bcrypt.hash(newPassword, 10);
    const { rows } = await query(`UPDATE users SET password_hash = $2, updated_at = NOW()
     WHERE id = $1 AND tenant_id IS NULL AND role IN ('super_admin', 'platform_admin')
     RETURNING id`, [req.params.id, hash]);
    if (!rows[0])
        return error(res, 'System user not found', 404);
    return success(res, { reset: true, temporaryPassword: newPassword });
});
...
router.post('/tenants', async (req, res) => {
    ...
    if (!name || !slug)
        return error(res, 'name and slug required');
    const normalizedSlug = String(slug).trim().toLowerCase().replace(/\s+/g, '-');
    const assignedManagerId = req.user?.role === 'platform_admin'
        ? req.user.id
    ...
    const wialonUserIds = Array.isArray(b.wialonUserIds)
        ? b.wialonUserIds.map((id) => parseInt(String(id), 10)).filter((n) => !Number.isNaN(n))
        : undefined;
    try {
        const tenantId = await withTransaction(async (tx) => {
            const { rows } = await tx(`INSERT INTO tenants (name, slug, primary_color, logo_url, favicon_url, contact_email, timezone, language, status, is_active, assigned_manager_id)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, 'draft', false, $9) RETURNING *`, [name, normalizedSlug, primaryColor || '#004225', normalizeUploadPath(logoUrl), normalizeUploadPath(faviconUrl), contactEmail, timezone || 'UTC', language || 'en', assignedManagerId]);
            const id = rows[0].id;
            await tx(`INSERT INTO tenant_modules (id, tenant_id, module_key, is_enabled, is_visible)
         SELECT UUID(), $1, m.\`key\`, m.default_enabled, 1
         FROM module_definitions m
         ON CONFLICT DO NOTHING`, [id]);
            await tx(`INSERT INTO tenant_backup_settings (tenant_id) VALUES ($1) ON CONFLICT DO NOTHING`, [id]);
            return id;
        });
        ...
        if (!Number.isNaN(wialonAccountId)) {
            ...
            wialonLink = await WialonAccountLinkService.linkAccount(tenantId, wialonAccountId, wialonAccountName, wialonUserIds, wialonMotherAccountId);
        }
        await AuditService.log({
            userId: req.user?.id,
            userEmail: req.user?.email,
        ...
        return success(res, { ...mapTenant(rows[0]), wialon: wialonLink }, 201);
    }
    catch (err) {
        const msg = err.message || '';
        if (msg.includes('tenants_status_check')) {
            return error(res, 'Database migration required: run `npm run db:migrate` to enable draft tenant status (migration 012).', 500);
        }
...
router.get('/tenants/:id', async (req, res) => {
    ...
    return success(res, { ...mapTenant(detail), usage: detail.usage });
});
router.patch('/tenants/:id', async (req, res) => {
    ...
    if (b.status === 'active')
        isActive = true;
    else if (b.status === 'draft' || b.status === 'inactive' || b.status === 'suspended')
        isActive = false;
    ...
    const faviconUrl = setFavicon
        ? b.faviconUrl
            ...
            : null
        : null;
    const { rows } = await query(`UPDATE tenants SET
    ...
    return success(res, mapTenant(rows[0]));
});
...
router.post('/tenants/bulk', async (req, res) => {
    ...
    if (!Array.isArray(tenantIds))
        return error(res, 'tenantIds required');
    const status = action === 'activate' ? 'active' : 'inactive';
    await query(`UPDATE tenants SET status = $2, is_active = $3 WHERE id = ANY($1)`, [tenantIds, status, action === 'activate']);
    return success(res, { updated: tenantIds.length });
});
// ─── Integrations ────────────────────────────────────────────────────────────
router.get('/tenants/:id/integrations', async (req, res) => {
    const { rows } = await query(`SELECT ds.id, ds.tenant_id, ds.source_type, ds.is_active, ds.last_sync_at, ds.last_error,
     ...
     WHERE ds.tenant_id = $1`, [req.params.id]);
    for (const r of rows) {
        const row = r;
        if (row.source_type === 'wialon' &&
            isWialonTenantConnected({
                is_active: Boolean(row.is_active),
                connection_verified_at: row.connection_verified_at,
                wialon_resource_id: row.wialon_resource_id != null ? Number(row.wialon_resource_id) : null,
            }) &&
            row.last_error) {
            await query(`UPDATE data_sources SET last_error = NULL, updated_at = NOW()
         WHERE tenant_id = $1 AND source_type = 'wialon'`, [req.params.id]);
            row.last_error = null;
        }
    }
    const { rows: tenant } = await query(`SELECT slug FROM tenants WHERE id = $1`, [req.params.id]);
...
router.put('/tenants/:id/integrations/:sourceType', async (req, res) => {
    ...
    const { rows: existingRows } = await query(`SELECT credentials_encrypted FROM data_sources WHERE tenant_id = $1 AND source_type = $2`, [req.params.id, sourceType]);
    if (existingRows[0]) {
        const prev = decryptCredentials(existingRows[0].credentials_encrypted);
        for (const key of Object.keys(credentials)) {
            const val = credentials[key];
            if (val === '' || val === undefined || val === null) {
                if (prev[key] !== undefined)
                    credentials[key] = prev[key];
                else
    ...
    if (sourceType === 'wialon' && credentials.token) {
        ...
        if (credentials.operateAs === '' || credentials.operateAs === null)
            delete credentials.operateAs;
        if (credentials.accountId === '' || credentials.accountId === null)
            delete credentials.accountId;
        if (credentials.baseUrl)
            credentials.baseUrl = String(credentials.baseUrl).trim();
    }
    ...
    const hasRequiredCreds = sourceType === 'wialon'
        ? !!credentials.token
        : sourceType === 'loconav'
    ...
    let assets = [];
    let wialonMeta = null;
    let wialonResourceId = null;
    let wialonOperateAs = null;
    let wialonAccountName = null;
    ...
    try {
        ...
        return error(res, `Connection failed: ${e.message}`);
    }
    finally {
    ...
    const { rows } = await query(`INSERT INTO data_sources (tenant_id, source_type, credentials_encrypted, is_active, sync_interval_minutes, webhook_secret)
     ...
     RETURNING id, tenant_id, source_type, is_active, last_sync_at, sync_interval_minutes`, [req.params.id, sourceType, encrypted, syncInterval, webhookSecret]);
    await query(`UPDATE data_sources SET connection_verified_at = NOW(), preview_asset_count = $3,
...
router.post('/tenants/:id/integrations/:sourceType/test', async (req, res) => {
    try {
        const result = await AdminOrchestrator.testIntegration(String(req.params.id), req.params.sourceType, req.body.credentials);
        return success(res, result);
    }
    catch (e) {
        return error(res, e.message);
...
router.post('/tenants/:id/integrations/:sourceType/sync', async (req, res) => {
    try {
        const result = await AdminOrchestrator.syncIntegration(String(req.params.id), req.params.sourceType);
        await AuditService.log({
            tenantId: String(req.params.id),
            userId: req.user?.id,
            userEmail: req.user?.email,
        ...
        await AuditService.logActivity(String(req.params.id), 'integration_sync', `${req.params.sourceType} sync completed`, `${result.vehiclesSynced} vehicles synced`);
        return success(res, result);
    }
    catch (e) {
        return error(res, e.message);
...
router.get('/tenants/:id/modules', async (req, res) => {
    const { rows } = await query(`SELECT md.\`key\`, md.label, md.description, md.icon, md.sources,
     ...
     ORDER BY md.sort_order`, [req.params.id]);
    return success(res, rows);
});
...
router.put('/tenants/:id/fuel-module-config', async (req, res) => {
    try {
...
router.post('/tenants/:id/fuel-station-sheets', async (req, res) => {
    try {
        const { fileName, mimeType, data, notes } = req.body;
        if (!fileName || !data)
            return error(res, 'fileName and data (base64) required');
        const base64 = data.includes(',') ? data.split(',')[1] : data;
        const buffer = Buffer.from(base64, 'base64');
        if (!buffer.length)
            return error(res, 'Empty file');
        const { FuelStationSheetService } = await import('../services/FuelStationSheetService.js');
...
router.delete('/tenants/:id/fuel-station-sheets/:uploadId', async (req, res) => {
    try {
        const { FuelStationSheetService } = await import('../services/FuelStationSheetService.js');
...
router.put('/tenants/:id/modules', async (req, res) => {
    ...
    if (!Array.isArray(modules))
        return error(res, 'modules array required');
    for (const m of modules) {
        await query(`INSERT INTO tenant_modules (tenant_id, module_key, is_enabled, is_visible)
       VALUES ($1, $2, $3, $4)
       ON CONFLICT (tenant_id, module_key) DO UPDATE SET is_enabled = $3, is_visible = $4`, [req.params.id, m.key, m.isEnabled, m.isVisible ?? true]);
    }
    await AuditService.log({
        tenantId: String(req.params.id),
        userId: req.user?.id,
...
router.get('/tenants/:id/users', async (req, res) => {
    const { rows } = await query(`SELECT u.id, u.email, u.full_name, u.role, u.is_active, u.last_login_at, u.created_at,
     ...
     FROM users u WHERE u.tenant_id = $1 ORDER BY u.full_name`, [req.params.id]);
    return success(res, rows);
});
router.get('/tenants/:id/users/:userId', async (req, res) => {
    const userId = String(req.params.userId);
    const { rows } = await query(`SELECT u.id, u.email, u.full_name, u.role, u.is_active, u.last_login_at, u.created_at
     FROM users u WHERE u.id = $2 AND u.tenant_id = $1`, [req.params.id, userId]);
    if (!rows[0])
        return error(res, 'User not found', 404);
    const modules = await getUserModules(userId);
    return success(res, { ...rows[0], modules });
...
router.post('/tenants/:id/users', async (req, res) => {
    ...
    const { rows } = await query(`INSERT INTO users (tenant_id, email, password_hash, full_name, role)
     VALUES ($1, $2, $3, $4, $5) RETURNING id, email, full_name, role, is_active`, [req.params.id, email, hash, fullName || email, role || 'viewer']);
    if (Array.isArray(modules)) {
        for (const mod of modules) {
            await query(`INSERT INTO user_modules (user_id, module_key, is_enabled) VALUES ($1, $2, true) ON CONFLICT DO NOTHING`, [rows[0].id, mod]);
        }
    }
    await AuditService.log({
        tenantId: String(req.params.id),
...
router.get('/tenants/:id/wialon/users', async (req, res) => {
    try {
        ...
        if (!accountId)
            return error(res, 'No Wialon account linked for this tenant', 400);
        const [wialonUsers, provisionedRows] = await Promise.all([
            WialonHierarchyService.getUsersForAccount(creds, accountId),
            query(`SELECT id, wialon_user_id FROM users WHERE tenant_id = $1 AND wialon_user_id IS NOT NULL`, [tenantId]),
        ]);
        const byWialonId = new Map(provisionedRows.rows.map((r) => [Number(r.wialon_user_id), r.id]));
        return success(res, {
            accountId,
...
router.post('/tenants/:id/users/from-wialon', async (req, res) => {
    try {
        const tenantId = String(req.params.id);
        ...
        if (!user)
            return error(res, 'Failed to provision user');
        ...
        if (Array.isArray(modules)) {
            ...
            for (const mod of modules) {
                await query(`INSERT INTO user_modules (user_id, module_key, is_enabled) VALUES ($1, $2, true) ON CONFLICT DO NOTHING`, [user.id, mod]);
            }
        }
        await AuditService.log({
            tenantId,
...
router.patch('/tenants/:tenantId/users/:userId', async (req, res) => {
    ...
    const { rows } = await query(`UPDATE users SET is_active = COALESCE($3, is_active), role = COALESCE($4, role),
                      full_name = COALESCE($5, full_name), updated_at = NOW()
     WHERE id = $2 AND tenant_id = $1 RETURNING id, email, full_name, role, is_active`, [req.params.tenantId, userId, isActive, role, fullName]);
    if (!rows[0])
        return error(res, 'User not found', 404);
    if (Array.isArray(modules)) {
        await query(`DELETE FROM user_modules WHERE user_id = $1`, [userId]);
...
router.post('/tenants/:id/import', async (req, res) => {
    const { data, skipDuplicates } = req.body;
    if (!data || typeof data !== 'object')
        return error(res, 'data object required');
    let imported = 0;
    if (Array.isArray(data.drivers)) {
        for (const d of data.drivers) {
            try {
                await query(`INSERT INTO drivers (tenant_id, name, license_number, phone, email, status)
           VALUES ($1, $2, $3, $4, $5, $6) ON CONFLICT (tenant_id, license_number) DO ${skipDuplicates ? 'NOTHING' : 'UPDATE SET name = EXCLUDED.name'}`, [req.params.id, d.name, d.license_number || d.licenseNumber, d.phone || '', d.email, d.status || 'available']);
                imported++;
            }
            catch { /* skip bad rows */ }
        }
...
router.get('/tenants/:id/backups', async (req, res) => {
    const { rows } = await query(`SELECT * FROM tenant_backups WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT 50`, [req.params.id]);
    const { rows: settings } = await query(`SELECT * FROM tenant_backup_settings WHERE tenant_id = $1`, [req.params.id]);
...
router.put('/tenants/:id/backups/settings', async (req, res) => {
    ...
    const { rows } = await query(`INSERT INTO tenant_backup_settings (tenant_id, auto_backup, frequency, backup_time, retention_days, updated_at)
     ...
     ON CONFLICT (tenant_id) DO UPDATE SET
       ...
       updated_at = NOW()
     RETURNING *`, [req.params.id, autoBackup, frequency, backupTime, retentionDays]);
    return success(res, rows[0]);
});
router.post('/tenants/:id/backups', async (req, res) => {
    try {
...
router.get('/tenants/:id/audit', async (req, res) => {
    ...
    const { rows } = await query(`SELECT * FROM audit_logs ${where} ORDER BY created_at DESC LIMIT 100`, params);
    return success(res, rows);
});
router.get('/audit', async (req, res) => {
    const { rows } = await query(`SELECT al.*, t.name as tenant_name FROM audit_logs al
...
router.get('/tenants/:id/api-keys', async (req, res) => {
    const { rows } = await query(`SELECT id, name, key_prefix, permissions, expires_at, last_used_at, is_active, created_at
     FROM tenant_api_keys WHERE tenant_id = $1 ORDER BY created_at DESC`, [req.params.id]);
    return success(res, rows);
});
router.post('/tenants/:id/api-keys', async (req, res) => {
    ...
    const expiresAt = expiresInDays
        ...
        : null;
    const { rows } = await query(`INSERT INTO tenant_api_keys (tenant_id, name, key_prefix, key_hash, permissions, expires_at)
     VALUES ($1, $2, $3, $4, $5, $6) RETURNING id, name, key_prefix, permissions, expires_at, created_at`, [req.params.id, name, prefix, hash, permissions || ['read'], expiresAt]);
    await AuditService.log({
        tenantId: String(req.params.id),
        userId: req.user?.id,
        userEmail: req.user?.email,
