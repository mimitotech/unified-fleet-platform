...
function findMysqlSocket() {
    if (process.env.DB_SOCKET?.trim())
        return process.env.DB_SOCKET.trim();
    const candidates = [
...
function readParts() {
    ...
    if ((!user || !database || !password) && process.env.DATABASE_URL?.startsWith('mysql')) {
        const u = new URL(process.env.DATABASE_URL.replace(/^mysql2:\/\//, 'mysql://'));
...
async function createWorkingPool() {
    ...
    if (socket) {
        try {
            ...
            errors.push(`socket ${socket}: ${err.message}`);
        }
    }
    else {
    ...
    try {
        ...
        errors.push(`tcp 127.0.0.1: ${err.message}`);
    }
    console.error('[mams-db] all connection strategies failed:\n - ' + errors.join('\n - '));
    throw new Error(`MySQL Access denied. passwordLength=${parts.password.length} fingerprint=${fp}. ` +
...
export function normalizeSql(text) {
    ...
    sql = sql.replace(/ON CONFLICT\s*\(([^)]+)\)\s*WHERE[\s\S]*?DO UPDATE\s+SET/gi, 'ON DUPLICATE KEY UPDATE');
    ...
    sql = sql.replace(/string_agg\s*\(\s*([\s\S]+?)\s*,\s*'([^']*)'\s*\)/gi, "GROUP_CONCAT($1 SEPARATOR '$2')");
    sql = sql.replace(/json_agg\s*\(\s*([\s\S]+?)\s*\)/gi, 'JSON_ARRAYAGG($1)');
    // ORDER BY col ASC NULLS LAST → ORDER BY (col IS NULL), col ASC
    sql = sql.replace(/(\S+)\s+(ASC|DESC)\s+NULLS\s+LAST\b/gi, '($1 IS NULL), $1 $2');
    sql = sql.replace(/(\S+)\s+(ASC|DESC)\s+NULLS\s+FIRST\b/gi, '($1 IS NOT NULL), $1 $2');
...
function rewriteDateTrunc(sql) {
    ...
    while ((m = re.exec(sql)) !== null) {
        ...
        if (unit === 'day')
            ...
            replacement = `DATE_FORMAT(${expr}, '%Y-%m-%d %H:00:00')`;
        else if (unit === 'month')
            replacement = `DATE_FORMAT(${expr}, '%Y-%m-01')`;
        else if (unit === 'year')
            replacement = `DATE_FORMAT(${expr}, '%Y-01-01')`;
        else
            replacement = `DATE(${expr})`;
...
function rewriteFilterAggregates(sql) {
    // COALESCE(SUM(x) FILTER (WHERE …), 0)
    sql = replaceAllBalanced(sql, /COALESCE\(\s*SUM\(([^)]+)\)\s*FILTER\s*\(\s*WHERE\s+/gi, (m, whereClause) => `COALESCE(SUM(CASE WHEN ${whereClause} THEN ${m[1]} ELSE 0 END), 0)`);
    sql = replaceAllBalanced(sql, /COUNT\(DISTINCT\s+(\w+(?:\.\w+)?)\)\s*FILTER\s*\(\s*WHERE\s+/gi, (m, whereClause) => `COUNT(DISTINCT CASE WHEN ${whereClause} THEN ${m[1]} END)`);
    sql = replaceAllBalanced(sql, /COUNT\(\*\)\s*FILTER\s*\(\s*WHERE\s+/gi, (_m, whereClause) => `SUM(CASE WHEN ${whereClause} THEN 1 ELSE 0 END)`);
    sql = replaceAllBalanced(sql, /SUM\(([^)]+)\)\s*FILTER\s*\(\s*WHERE\s+/gi, (m, whereClause) => `SUM(CASE WHEN ${whereClause} THEN ${m[1]} ELSE 0 END)`);
    return sql;
}
/**
 ...
 */
function replaceAllBalanced(sql, prefixRe, build) {
    let out = '';
    let last = 0;
...
export function prepareSql(text, params = []) {
    let src = text;
    const anyValues = [];
    src = src.replace(/=\s*ANY\s*\(\s*\$(\d+)(?:::[^)]*)?\s*\)/gi, (_full, n) => {
...
async function executeRaw(conn, text, params = []) {
    const prepared = prepareSql(text, params);
    const [result] = await conn.execute(prepared.sql, prepared.params);
...
async function queryWithConn(conn, text, params = []) {
    const returningMatch = text.match(/\sRETURNING\s+([\s\S]+)$/i);
    const selectCols = returningMatch ? returningMatch[1].trim() : '*';
    let sql = returningMatch && returningMatch.index != null ? text.slice(0, returningMatch.index).trim() : text;
    const bind = [...params];
    if (returningMatch) {
        ...
        if (insert) {
            ...
            const valsMatch = sql.match(/VALUES\s*\(([\s\S]*?)\)\s*(?:ON\s+(?:DUPLICATE|CONFLICT)|RETURNING|$)/i);
            const valueParts = valsMatch?.[1].split(',').map((v) => v.trim()) ?? [];
            const bindForCol = (colName) => {
                ...
                if (dollar)
                    return bind[Number(dollar[1]) - 1];
                if (ph && /^'[0-9a-f-]{36}'$/i.test(ph))
                    return ph.slice(1, -1);
                return undefined;
            };
            // Natural / alternate PKs — never invent an `id` column (e.g. user_preferences.user_id)
            let reselectCol = null;
            let reselectVal;
            for (const key of ['user_id', 'token', 'key', 'slug']) {
            ...
            if (!reselectCol) {
                ...
                if (!id && !cols.some((c) => c.toLowerCase() === 'id')) {
                    id = randomUUID();
                    sql = sql.replace(/INSERT\s+(IGNORE\s+)?INTO\s+`?(\w+)`?\s*\(([^)]+)\)\s*VALUES\s*\(/i, `INSERT ${ignore}INTO ${table} (id, ${colList}) VALUES ('${id}', `);
                }
                if (id) {
                    reselectCol = 'id';
                    reselectVal = id;
            ...
            if (reselectCol != null && reselectVal != null) {
                return executeRaw(conn, `SELECT ${selectCols === '*' ? '*' : selectCols} FROM ${table} WHERE \`${reselectCol}\` = $1`, [reselectVal]);
            }
            if (execResult.insertId) {
                return executeRaw(conn, `SELECT ${selectCols === '*' ? '*' : selectCols} FROM ${table} WHERE id = $1`, [execResult.insertId]);
            }
        ...
        if (tableMatch && whereMatch) {
            ...
            return executeRaw(conn, `SELECT ${selectCols === '*' ? '*' : selectCols} FROM ${table} WHERE ${whereMatch[1]}`, bind);
        }
    }
...
export async function query(text, params = []) {
    return queryWithConn(getPool(), text, params);
}
export async function withTransaction(fn) {
    const conn = await getPool().getConnection();
    try {
        await conn.beginTransaction();
        const txQuery = (async (text, params = []) => queryWithConn(conn, text, params));
