...
export class AdminOrchestrator {
    static async getDashboardStats() {
        const { rows: tenantStats } = await query(`SELECT COUNT(*)::int as total,
       ...
       FROM tenants`);
        const { rows: vehicleStats } = await query(`SELECT COUNT(*) as total,
              SUM(CASE WHEN (
       ...
       FROM assets a`);
        const { rows: userStats } = await query(`SELECT COUNT(*)::int as total FROM users WHERE role != 'platform_admin'`);
        const { rows: alertStats } = await query(`SELECT COUNT(*)::int as pending FROM alerts WHERE acknowledged = false`);
        const { rows: integrationStats } = await query(`SELECT COUNT(*)::int as total, COUNT(*) FILTER (WHERE is_active = true)::int as active FROM data_sources`);
        const totalIntegrations = Number(integrationStats[0]?.total || 0);
        const activeIntegrations = Number(integrationStats[0]?.active || 0);
        const integrationHealth = totalIntegrations > 0 ? Math.round((activeIntegrations / totalIntegrations) * 1000) / 10 : 100;
        const { rows: healthHistory } = await query(`SELECT date_trunc('day', started_at)::date::text as day,
              ROUND(AVG(CASE WHEN status = 'success' THEN 100 ELSE 0 END)::numeric, 1)::float as score
       FROM integration_sync_logs
       WHERE started_at >= NOW() - INTERVAL '7 days'
       GROUP BY 1 ORDER BY 1`);
        const { rows: growthHistory } = await query(`SELECT date_trunc('day', created_at)::date::text as day, COUNT(*)::int as count
       FROM tenants WHERE created_at >= NOW() - INTERVAL '30 days'
       GROUP BY 1 ORDER BY 1`);
        const { rows: recentActivity } = await query(`SELECT af.*, t.name as tenant_name FROM activity_feed af
       LEFT JOIN tenants t ON t.id = af.tenant_id
       ORDER BY af.created_at DESC LIMIT 10`);
        const { rows: topTenants } = await query(`SELECT t.id, t.name, t.slug, t.status,
              CASE
                WHEN EXISTS (
       ...
       GROUP BY t.id ORDER BY vehicle_count DESC LIMIT 5`);
        const { rows: recentIncidents } = await query(`SELECT isl.*, t.name as tenant_name FROM integration_sync_logs isl
       JOIN tenants t ON t.id = isl.tenant_id
       WHERE isl.status = 'failed'
       ORDER BY isl.started_at DESC LIMIT 5`);
        const { rows: assetStatusBreakdown } = await query(`SELECT COALESCE((
       ...
       FROM assets a
       GROUP BY 1`);
        const { rows: alertsTimeline } = await query(`SELECT date_trunc('hour', occurred_at)::timestamptz::text as hour,
       ...
       GROUP BY 1 ORDER BY 1`);
        const { rows: alertsBySeverity } = await query(`SELECT severity, COUNT(*)::int as count
       FROM alerts
       WHERE occurred_at >= NOW() - INTERVAL '7 days'
       GROUP BY severity ORDER BY count DESC`);
        const { rows: syncTimeline } = await query(`SELECT date_trunc('day', started_at)::date::text as day,
              COUNT(*) FILTER (WHERE status = 'success')::int as success,
              COUNT(*) FILTER (WHERE status = 'failed')::int as failed
       FROM integration_sync_logs
       WHERE started_at >= NOW() - INTERVAL '7 days'
       GROUP BY 1 ORDER BY 1`);
        const { rows: integrationsBySource } = await query(`SELECT source_type::text, COUNT(*)::int as total,
              COUNT(*) FILTER (WHERE is_active = true)::int as active
       FROM data_sources
       GROUP BY source_type ORDER BY total DESC`);
        const { rows: tenantStatusBreakdown } = await query(`SELECT COALESCE(status, CASE WHEN is_active THEN 'active' ELSE 'inactive' END) as status,
              COUNT(*)::int as count
       FROM tenants
       GROUP BY 1`);
        const { rows: loginStats } = await query(`SELECT COUNT(*) FILTER (WHERE last_login_at >= NOW() - INTERVAL '24 hours')::int as logins24h,
              COUNT(*) FILTER (WHERE last_login_at >= NOW() - INTERVAL '7 days')::int as activeUsers7d
       FROM users WHERE is_active = true`);
        const { rows: webhookStats } = await query(`SELECT COUNT(*)::int as events24h FROM alerts
       WHERE source_type IN ('loconav', 'tracksolid') AND occurred_at >= NOW() - INTERVAL '24 hours'`);
        const { rows: syncStats24h } = await query(`SELECT COUNT(*)::int as total,
              COUNT(*) FILTER (WHERE status = 'success')::int as success,
              COALESCE(SUM(vehicles_synced) FILTER (WHERE status = 'success'), 0)::int as assetsSynced
       FROM integration_sync_logs
       WHERE started_at >= NOW() - INTERVAL '24 hours'`);
        const { rows: recentSyncs } = await query(`SELECT isl.source_type, isl.status, isl.vehicles_synced, isl.started_at, isl.message, t.name as tenant_name
    ...
    static async getLastSyncTime() {
        const { rows } = await query(`SELECT MAX(last_sync_at)::text as last_sync FROM data_sources`);
    ...
    static async getSystemHealth() {
        ...
        const { rows: sourceHealth } = await query(`SELECT ds.source_type, ds.is_active, ds.last_sync_at, ds.last_error, t.name as tenant_name,
       ...
       GROUP BY ds.id, t.name`);
        const { rows: webhookCount } = await query(`SELECT COUNT(*)::int as count FROM alerts
       WHERE source_type IN ('loconav','tracksolid') AND occurred_at >= NOW() - INTERVAL '24 hours'`);
        return {
            ...
            recentIncidents: await query(`SELECT * FROM integration_sync_logs WHERE status = 'failed' ORDER BY started_at DESC LIMIT 10`).then((r) => r.rows),
        };
    }
    static async listTenantsWithStats(filters) {
       ...
       LIMIT $${params.length + 1} OFFSET $${params.length + 2}`, [...params, limit, offset]);
        const { rows: countRows } = await query(`SELECT COUNT(*)::int as total FROM tenants t ${where}`, params);
        const tenants = rows;
        let byManager;
        if (filters.groupByManager) {
            const groups = new Map();
            for (const row of rows) {
                const managerId = row.assigned_manager_id || null;
                const managerName = row.assigned_manager_name || 'Unassigned';
    ...
    static async getTenantDetail(tenantId) {
        const { rows } = await query(`SELECT t.*, mgr.full_name as assigned_manager_name, mgr.email as assigned_manager_email
       FROM tenants t
       LEFT JOIN users mgr ON mgr.id = t.assigned_manager_id
       WHERE t.id = $1`, [tenantId]);
        if (!rows[0])
            return null;
        const { rows: usage } = await query(`SELECT
         (SELECT COUNT(*)::int FROM assets WHERE tenant_id = $1) as vehicles_used,
         (SELECT COUNT(*)::int FROM users WHERE tenant_id = $1) as users_used`, [tenantId]);
        return { ...rows[0], usage: usage[0] };
    }
    static async syncIntegration(tenantId, sourceType) {
        ...
        try {
            ...
            if (sourceType === 'wialon') {
                ...
                const userPart = result.usersCreated != null
            ...
            await query(`UPDATE data_sources SET last_sync_at = NOW(), last_error = NULL WHERE tenant_id = $1 AND source_type = $2`, [tenantId, sourceType]);
            await query(`INSERT INTO integration_sync_logs (tenant_id, source_type, status, message, vehicles_synced, completed_at)
         VALUES ($1, $2, 'success', $3, $4, NOW())`, [tenantId, sourceType, message, vehiclesSynced]);
            await AssetOrchestrator.invalidateTenantCache(tenantId);
            return { success: true, vehiclesSynced, startedAt: started, message };
        }
        catch (err) {
            const msg = err.message;
            await query(`UPDATE data_sources SET last_error = $3 WHERE tenant_id = $1 AND source_type = $2`, [tenantId, sourceType, msg]);
            await query(`INSERT INTO integration_sync_logs (tenant_id, source_type, status, message, completed_at)
         VALUES ($1, $2, 'failed', $3, NOW())`, [tenantId, sourceType, msg]);
            throw err;
        }
    }
    static async testIntegration(tenantId, sourceType, credentials) {
        let creds = credentials;
        if (!creds) {
            if (sourceType === 'wialon') {
                const { loadTenantWialonCreds } = await import('../services/tenantWialonCredentials.js');
                creds = (await loadTenantWialonCreds(tenantId));
            }
            else {
                const { rows } = await query(`SELECT credentials_encrypted FROM data_sources WHERE tenant_id = $1 AND source_type = $2`, [tenantId, sourceType]);
                if (!rows[0])
        ...
        if (sourceType === 'tracksolid' && creds.password && typeof creds.password === 'string') {
            creds = { ...creds };
            creds.passwordMd5 = crypto.createHash('md5').update(creds.password).digest('hex');
       ...
       WHERE tenant_id = $1 AND source_type = $2`, [tenantId, sourceType, assets.length, JSON.stringify(sampleAssets)]);
        return {
            connected: true,
            sourceType,
    ...
    static async getRecommendedModules(tenantId) {
        const { rows: sources } = await query(`SELECT source_type FROM data_sources WHERE tenant_id = $1 AND is_active = true AND connection_verified_at IS NOT NULL`, [tenantId]);
        const connected = sources.map((s) => s.source_type);
        const { rows: modules } = await query(`SELECT \`key\`, label, sources FROM module_definitions ORDER BY sort_order`);
        return modules.map((m) => {
            const requiredSources = m.sources || [];
            const canEnable = requiredSources.length === 0 ||
                requiredSources.some((s) => connected.includes(s));
            return {
                moduleKey: m.key,
                label: m.label,
                requiredSources,
                ...
                reason: canEnable
                    ? requiredSources.length === 0
                        ...
                        : `Requires ${requiredSources.filter((s) => connected.includes(s)).join(' or ')}`
                    : `Needs ${requiredSources.join(' or ')} — not connected yet`,
            };
        });
    }
    static async activateTenant(tenantId) {
        const { rows: existing } = await query(`SELECT id, name, status FROM tenants WHERE id = $1`, [tenantId]);
        if (!existing[0])
            throw new Error('Tenant not found');
        const { rows: verified } = await query(`SELECT COUNT(*)::int as count FROM data_sources
       WHERE tenant_id = $1 AND is_active = true AND connection_verified_at IS NOT NULL`, [tenantId]);
        const verifiedIntegrations = Number(verified[0]?.count || 0);
        await query(`UPDATE tenants SET status = 'active', is_active = true, updated_at = NOW() WHERE id = $1`, [tenantId]);
        await AssetOrchestrator.invalidateTenantCache(tenantId);
        const warnings = [];
        if (verifiedIntegrations === 0) {
            warnings.push('No telematics systems verified yet. Users can sign in, but live fleet data will appear after you connect and test an integration.');
        }
        return {
            status: 'active',
    ...
    static async createBackup(tenantId, backupType = 'full') {
        const { rows } = await query(`INSERT INTO tenant_backups (tenant_id, backup_type, status) VALUES ($1, $2, 'running') RETURNING id`, [tenantId, backupType]);
        const backupId = rows[0].id;
        try {
            const tables = ['assets', 'drivers', 'fleet_routes', 'alerts', 'fuel_transactions', 'geofences'];
            let totalRows = 0;
            for (const table of tables) {
                const { rows: cnt } = await query(`SELECT COUNT(*)::int as c FROM ${table} WHERE tenant_id = $1`, [tenantId]);
                totalRows += Number(cnt[0]?.c || 0);
            }
            const sizeBytes = totalRows * 1024;
            await query(`UPDATE tenant_backups SET status = 'success', size_bytes = $2, completed_at = NOW(),
         file_path = $3 WHERE id = $1`, [backupId, sizeBytes, `/backups/${tenantId}/${backupId}.json`]);
            return { id: backupId, status: 'success', sizeBytes, rows: totalRows };
        }
        catch (err) {
            await query(`UPDATE tenant_backups SET status = 'failed', error_message = $2, completed_at = NOW() WHERE id = $1`, [backupId, err.message]);
            throw err;
        }
    }
    static async exportTenantData(tenantId, include = ['all']) {
        const exportAll = include.includes('all');
        const data = { exportedAt: new Date().toISOString(), tenantId };
        ...
        if (exportAll || include.includes('users')) {
            const { rows } = await query(`SELECT id, email, full_name, role, is_active, created_at FROM users WHERE tenant_id = $1`, [tenantId]);
            data.users = rows;
        }
