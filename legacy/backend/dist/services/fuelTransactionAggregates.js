...
import { exactRangeSummaryUnitIds, isExactRangeSummary, isWialonGroupSummary, } from './wialonFuelReport/rangeFilter.js';
/**
 ...
 */
export function computeFuelKpis(rows, fromDate, toDate) {
    ...
    for (const [unitId, unitRows] of byUnit) {
        ...
        if (exactSummaries.length && exactSummaryIds.has(unitId)) {
            for (const t of exactSummaries) {
                if (t.filled > summaryFilled)
                    summaryFilled = t.filled;
                if (t.fuelUsed > summaryUsed)
                    summaryUsed = t.fuelUsed;
...
export function monthlyFuelTrend(rows) {
    const byMonth = new Map();
    for (const r of rows) {
        if (!r.timestamp)
            continue;
        if (isWialonGroupSummary(r) || r.sensor === 'balance')
            continue;
        const month = new Date(r.timestamp * 1000).toISOString().slice(0, 7);
        const row = byMonth.get(month) ?? { filled: 0, consumed: 0 };
        if (r.section === 'filling')
            row.filled += effectiveFilled(r);
        if (r.section === 'consumption')
            row.consumed += effectiveConsumed(r);
        if (r.section === 'theft')
            row.consumed += effectiveTheft(r);
...
export function enrichTankLevels(mapped) {
    const tankByKey = new Map();
    for (const r of mapped) {
        const key = `${r.unitId}:${r.timestamp}`;
        const cur = tankByKey.get(key) ?? {};
        if (r.tank === 'main' || !r.tank)
            cur.main = r.finalLevel;
        if (r.tank === 'reserve')
            cur.reserve = r.finalLevel;
...
export function splitDateRangeByDays(fromDate, toDate, chunkDays) {
    const out = [];
...
export function rollingFuelRange(days) {
    const to = new Date();
