...
export class WialonSyncService {
    static async syncTenant(tenantId, options) {
        await AssetOrchestrator.invalidateTenantCache(tenantId);
        const { rows: ds } = await query(`SELECT wialon_resource_id, wialon_account_name FROM data_sources
       WHERE tenant_id = $1 AND source_type = 'wialon' AND is_active = true`, [tenantId]);
        if (!ds[0]) {
        ...
        if (!units) {
            try {
                ...
                throw new Error(`Wialon unit fetch failed: ${err.message}`);
            }
        }
        if (units.length === 0) {
            const { rows: preview } = await query(`SELECT preview_asset_count FROM data_sources WHERE tenant_id = $1 AND source_type = 'wialon'`, [tenantId]);
            const expected = preview[0]?.preview_asset_count || 0;
            if (expected > 0) {
                throw new Error(`Wialon returned 0 units for this account (expected ~${expected}). Re-save the mother token in Wialon Center and sync again.`);
            }
        }
        ...
        try {
            ...
            try {
             ...
             ON CONFLICT (tenant_id, license_number) DO UPDATE SET name = EXCLUDED.name, phone = EXCLUDED.phone, updated_at = NOW()`, [tenantId, d.name, d.licenseNumber || d.id, d.phone || '', d.email || null]);
                    driversSynced++;
                }
            }
            catch {
            ...
            try {
                ...
                for (const z of zones) {
                    const { rows: existing } = await query(`SELECT id FROM geofences WHERE tenant_id = $1 AND name = $2 AND deleted_at IS NULL LIMIT 1`, [tenantId, z.name]);
                    if (existing[0]) {
                        await query(`UPDATE geofences SET type = $2, center = $3, radius = $4, points = $5, updated_at = NOW() WHERE id = $1`, [existing[0].id, z.type, z.center ? JSON.stringify(z.center) : null, z.radius, z.points ? JSON.stringify(z.points) : null]);
                    }
                    else {
                        await query(`INSERT INTO geofences (tenant_id, name, type, center, radius, points, color, is_active)
               VALUES ($1, $2, $3, $4, $5, $6, $7, true)`, [tenantId, z.name, z.type, z.center ? JSON.stringify(z.center) : null, z.radius, z.points ? JSON.stringify(z.points) : null, z.color || '#3B82F6']);
                    }
                    geofencesSynced++;
                }
            }
        ...
        let provision = { created: 0, updated: 0, deactivated: 0, users: [] };
        if (wialonUsers.length) {
            provision = await WialonUserProvisionService.provisionUsers(tenantId, accountId, wialonUsers, wialonUsers.map((u) => u.id));
        }
    ...
    static async pruneOutOfScopeAssets(tenantId, activeExternalIds) {
        if (!activeExternalIds.length)
            return;
        const { rows: staleMappings } = await query(`SELECT am.asset_id, am.external_id FROM asset_mappings am
       JOIN assets a ON a.id = am.asset_id
       WHERE a.tenant_id = $1 AND am.source_type = 'wialon'
         AND NOT (am.external_id = ANY($2::text[]))`, [tenantId, activeExternalIds]);
        for (const row of staleMappings) {
            ...
            const { rows: remaining } = await query(`SELECT COUNT(*)::int AS n FROM asset_mappings WHERE asset_id = $1`, [row.asset_id]);
