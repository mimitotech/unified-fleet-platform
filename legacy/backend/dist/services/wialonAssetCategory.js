...
function findCustomField(flds, ...names) {
    if (!flds)
        return '';
...
export const MACHINERY_NAME_RE = /excavat|crane|compressor|\bpump\b|welder|fork\s*lift|forklift|loader|bulldozer|roller|paver|mixer|grader|back\s*hoe|backhoe|shovel|drill(ing)?\s*rig|plant\b|machiner|\bequip/i;
export const GENERATOR_NAME_RE = /pearl\s*bank|genset|generator|\bdg\s*set\b|\bgen\s*set\b|\bkva\b|standby\s*power|power\s*pack|bowser|fuel\s*tanker/i;
...
export function isFuelBowserName(name) {
...
function readExplicitCategory(customFields, flds) {
    const raw = (customFields.asset_category ||
    ...
    if (!raw)
        return null;
    if (raw.includes('gen') || raw.includes('genset'))
        return 'generator';
    if (raw.includes('mach') || raw.includes('equip') || raw.includes('plant') || raw.includes('crane'))
        return 'machinery';
    if (raw.includes('veh') || raw.includes('truck') || raw.includes('lorry') || raw.includes('bus'))
        return 'vehicle';
    return null;
}
function hasEngineHoursSensor(sensorNames) {
...
export function isWialonGenerator(input) {
    ...
    if (input.unitId && input.groupMembership?.generatorUnitIds.has(input.unitId))
        return true;
    ...
    if (explicit === 'generator')
        return true;
    if (explicit === 'vehicle' || explicit === 'machinery')
        return false;
    ...
    if (isFuelBowserName(name))
        return true;
    const vehicleType = (cf.vehicle_type ||
        cf['Vehicle Type'] ||
        findCustomField(input.flds, 'vehicle_type', 'Vehicle Type')).toLowerCase();
    ...
    if (vehicleType.includes('gen') || unitType.includes('gen'))
        return true;
    if (model.includes('kva'))
        return true;
    // Soft heuristics only when Wialon structure supports generators
    if (GENERATOR_NAME_RE.test(nameLower))
        return true;
    const eh = input.engineHours ?? 0;
    const mi = input.mileage ?? 0;
    const sensors = input.sensorNames ?? [];
    if (hasEngineHoursSensor(sensors) && !plate && mi < 500)
        return true;
    if (!plate && eh > 50 && mi < 500 && (looksLikeSiteName(name) || eh > 200))
        return true;
    return false;
}
/**
...
export function isWialonMachinery(input) {
    ...
    if (input.unitId && input.groupMembership?.machineryUnitIds.has(input.unitId))
        return true;
    const cf = input.customFields || {};
    const explicit = readExplicitCategory(cf, input.flds);
    if (explicit === 'machinery')
        return true;
    if (explicit === 'vehicle' || explicit === 'generator')
        return false;
...
export function isWialonVehicle(input) {
    ...
    if (explicit === 'vehicle')
        return true;
    if (explicit === 'generator' || explicit === 'machinery')
        return false;
...
export function resolveFuelAssetCategory(input) {
    if (isWialonGenerator(input))
        return 'generator';
    if (isWialonMachinery(input))
        return 'machinery';
...
export function resolveAssetCategory(input) {
