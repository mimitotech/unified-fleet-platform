...
class FuelEventStore {
    byTenant = new Map();
    sessionFilled = new Map();
    tenantKey(tenantId, unitId) {
        return `${tenantId}:${unitId}`;
    }
    append(tenantId, event) {
    ...
    recordLevelSnapshots(tenantId, units) {
        const now = new Date().toISOString();
        for (const u of units) {
            if (u.levelLiters == null && !u.sensors?.length)
                continue;
    ...
    listFillEvents(tenantId, limit = 200) {
        return (this.byTenant.get(tenantId) ?? [])
            .filter((e) => e.type === 'filling' && !e.markedFalse)
            .slice(0, limit);
    }
    monthlyTrend(tenantId, days = 30) {
