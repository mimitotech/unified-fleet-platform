...
function buildEmail(login, wialonUserId, tenantSlug, wialonEmail) {
...
export class WialonUserProvisionService {
    ...
    static async provisionTenantUsers(tenantId, selectedUserIds) {
        ...
        const wialonUsers = selectedUserIds?.length
    ...
    static async provisionUsers(tenantId, accountId, wialonUsers, activeWialonIds) {
        const { rows: tenantRows } = await query(`SELECT slug FROM tenants WHERE id = $1`, [tenantId]);
        const tenantSlug = tenantRows[0]?.slug || 'tenant';
        const { rows: dsRows } = await query(`SELECT wialon_account_name FROM data_sources WHERE tenant_id = $1 AND source_type = 'wialon'`, [tenantId]);
        ...
        for (const wu of wialonUsers) {
            ...
            const { rows: byWialon } = await query(`SELECT id FROM users WHERE tenant_id = $1 AND wialon_user_id = $2 LIMIT 1`, [tenantId, wu.id]);
            if (byWialon[0]) {
                await query(`UPDATE users SET full_name = $3, wialon_login = $4, email = $5, is_active = true, updated_at = NOW()
           WHERE id = $1 AND tenant_id = $2`, [byWialon[0].id, tenantId, login, login, email]);
                updated++;
                users.push({
                    id: byWialon[0].id,
                    email,
            ...
            const { rows: byEmail } = await query(`SELECT id FROM users WHERE email = $1 LIMIT 1`, [email]);
            if (byEmail[0]) {
                await query(`UPDATE users SET tenant_id = $2, full_name = $3, wialon_user_id = $4, wialon_login = $5,
           role = $6, is_active = true, updated_at = NOW()
           WHERE id = $1`, [byEmail[0].id, tenantId, login, wu.id, login, role]);
                updated++;
                users.push({
                    id: byEmail[0].id,
                    email,
            ...
            const ins = await query(`INSERT INTO users (tenant_id, email, password_hash, full_name, role, wialon_user_id, wialon_login)
         VALUES ($1, $2, $3, $4, $5, $6, $7)
         RETURNING id`, [tenantId, email, passwordHash, login, role, wu.id, login]);
            created++;
            users.push({
                id: ins.rows[0].id,
                email,
        ...
        const { rows: stale } = await query(`SELECT id, wialon_user_id FROM users
       WHERE tenant_id = $1 AND wialon_user_id IS NOT NULL AND is_active = true`, [tenantId]);
        let deactivated = 0;
    ...
    static async provisionOneUser(tenantId, wialonUserId) {
        const creds = await loadTenantWialonCreds(tenantId);
        const accountId = Number(creds.accountId);
        if (!accountId)
            throw new Error('No Wialon account linked for this tenant');
        ...
        if (!wu)
            throw new Error('Wialon user not found in the linked account');
        const { rows: existing } = await query(`SELECT wialon_user_id FROM users
       WHERE tenant_id = $1 AND wialon_user_id IS NOT NULL AND is_active = true`, [tenantId]);
...
export class WialonAccountLinkService {
    static async linkAccount(tenantId, accountId, accountName, selectedUserIds, motherAccountId) {
        ...
        if (!motherId)
            throw new Error('Select a Wialon mother account before linking a tenant');
        const platformCreds = await import('./WialonMotherAccountService.js').then((m) => m.WialonMotherAccountService.loadCreds(motherId));
        ...
        await query(`INSERT INTO data_sources (tenant_id, source_type, credentials_encrypted, is_active, sync_interval_minutes, webhook_secret, inherits_platform_credentials)
       ...
       ON CONFLICT (tenant_id, source_type) DO UPDATE SET
         ...
         updated_at = NOW()`, [tenantId, tenantCreds, webhookSecret]);
        await query(`UPDATE data_sources SET
         ...
         last_error = NULL,
         updated_at = NOW()
       WHERE tenant_id = $1 AND source_type = 'wialon'`, [tenantId, accountId, resolvedName, motherId, JSON.stringify(wialonMeta), units.length]);
        const userIds = selectedUserIds?.length ? selectedUserIds : users.map((u) => u.id);
        let provision = { accountId, accountName: resolvedName, created: 0, updated: 0, deactivated: 0, users: [] };
        if (userIds.length) {
            provision = await WialonUserProvisionService.provisionUsers(tenantId, accountId, users.filter((u) => userIds.includes(u.id)), userIds);
        }
        let sync = {
        ...
        try {
            const syncResult = await import('./WialonSyncService.js').then((m) => m.WialonSyncService.syncTenant(tenantId, { credentials: creds, units }));
            sync = {
                ...syncResult,
            ...
            await query(`UPDATE data_sources SET last_error = NULL WHERE tenant_id = $1 AND source_type = 'wialon'`, [tenantId]);
        }
        catch (syncErr) {
            const warn = `Sync after link: ${syncErr.message}`;
        ...
        await import('../orchestrators/AssetOrchestrator.js').then((m) => m.AssetOrchestrator.invalidateTenantCache(tenantId));
        return {
            accountId,
