...
export class WialonFuelManagementService {
    ...
    static async syncLiveFuel(tenantId) {
        const snap = await WialonFleetService.getCachedLiveFleet(tenantId);
        let fillByUnit = new Map();
        try {
            const creds = await loadTenantWialonCreds(tenantId);
            const fls = await WialonFuelService.getFleetFuelLive(creds, snap.units.map((u) => u.id), new Map(snap.units.map((u) => [u.id, (u.sens ?? []).map((s) => ({ id: s.id, name: s.name }))])));
            fillByUnit = new Map(fls.filter((r) => (r.fuel?.filled ?? 0) > 0).map((r) => [r.unitId, r.fuel.filled]));
        }
        catch {
            /* FLS optional */
        }
        const units = snap.units
            ...
            const merged = fuel
                ...
                : undefined;
            const total = merged?.levelLiters ?? (merged?.sensors ? totalLitersFromReadings(merged.sensors.map((s, i) => ({
        ...
        wialonFuelEventStore.ingestFromLive(tenantId, units.map((u) => ({
            unitId: u.unitId,
            unitName: u.unitName,
    ...
    static async getUnitProfile(tenantId, unitId) {
        ...
        const levelParams = normalizeFuelLevelParams(settings.fuelLevelParams);
        return {
            unitId,
            settings: {
                ...settings,
