...
export function previousMonthKeyFrom(yyyyMm) {
...
export function buildComparison(current, previous, previousFrom, previousTo, previousMonth) {
    return {
        previousFrom,
        previousTo,
        previousMonth,
...
export function resolvePeriod(opts) {
    ...
    if (opts.period === 'custom' && opts.from && opts.to) {
        ...
        const granularity = days > 120 ? 'month' : days > 45 ? 'week' : 'day';
...
function fuelCosts(filled, consumed, lost, price) {
    if (price <= 0) {
...
function formatLevelDelta(initial, final, delta) {
...
function estimateOpeningFuel(rows) {
    const sorted = [...rows].filter((r) => r.timestamp).sort((a, b) => a.timestamp - b.timestamp);
    for (const r of sorted) {
        if (r.initialLevel > 0)
            return r.initialLevel;
        if (r.finalLevel > 0)
            return r.finalLevel;
    }
    return 0;
}
function estimateClosingFromRows(rows) {
...
export function computeFuelLedger(rows, totals, liveRemaining) {
    ...
    const computedRemaining = round1(Math.max(0, openingFuel + totalFilled - totalConsumed - totalLost));
...
function assetOpeningFuel(rows, unitId) {
...
export function buildAnalytics(rows, opts) {
    const filtered = opts.unitId != null ? rows.filter((r) => r.unitId === opts.unitId) : rows;
    ...
    const byAsset = [...assetMap.values()]
        ...
        return {
            ...
            cost: costs.fillCost,
            avgConsumption: a.mileage > 0 ? round1((a.consumed / a.mileage) * 100) : 0,
            sharePercent: totalConsumed > 0 ? round1((a.consumed / totalConsumed) * 100) : 0,
    ...
    const ledger = computeFuelLedger(filtered, { filled: totalFilled, consumed: totalConsumed, lost: totalTheft }, liveRemainingTotal);
    return {
        ...
        kpis: {
            ...
            fillEvents,
            consumptionEvents,
            unitsTracked: opts.unitId == null && opts.fleetUnits?.length
...
function detectAnomalies(rows, byAsset) {
    const out = [];
    const fleetAvg = byAsset.length > 0
...
function monthlyPointsFromRows(rows, price) {
    const map = new Map();
    for (const r of rows) {
        if (!r.timestamp)
            continue;
        ...
        if (r.section === 'filling')
            pt.filled += effectiveFilled(r);
        if (r.section === 'consumption') {
            pt.consumed += effectiveConsumed(r);
            pt.mileage += r.mileage || 0;
        }
        if (r.section === 'theft')
            pt.theft += effectiveTheft(r);
        map.set(key, pt);
    }
    return [...map.values()]
        .sort((a, b) => a.key.localeCompare(b.key))
...
function predictFromMonthly(monthlySeries) {
