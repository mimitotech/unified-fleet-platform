 ...
 */
export function isPlausibleFuelEvent(tx, liveLiters) {
    if (tx.sensor === 'balance')
        return false;
    ...
    const volume = Math.max(Number(tx.filled) || 0, Number(tx.suddenFuelDrop) || 0, Number(tx.fuelUsed) || 0);
    ...
    if (isSummary) {
        ...
        if (ref > 0 && volume > ref * 5 && volume > 2000)
            return false;
        if (volume > 100_000)
            return false;
        return true;
    ...
    if (ref > 0) {
        ...
        const softStationary = maxLevel > 0 && ref < 50 && maxLevel >= 100 && maxLevel < 50_000;
...
export function filterPlausibleFuelEvents(rows, liveFuelByUnit) {
...
export function isPlausibleBalanceLevel(level, liveLiters) {
    if (level <= 0)
        return false;
    if (liveLiters == null || liveLiters <= 0)
        return level <= 20_000;
