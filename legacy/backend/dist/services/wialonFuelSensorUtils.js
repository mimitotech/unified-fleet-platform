 ...
 */
export function isFuelLevelSensor(name, type) {
    const n = (name || '').toLowerCase().trim();
    const t = String(type ?? '').toLowerCase().trim();
    if (!n && !t)
        return false;
    if (EXCLUDE_NAME.test(n))
        return false;
    if (NON_FUEL_SENSOR.test(n))
        return false;
    // Canonical Wialon type first — works for every client locale.
    if (FUEL_LEVEL_TYPE_RE.test(t))
        return true;
    if (/^fuel level\b/i.test(n))
        return true;
    if (/^(?:fls|lls)\d*$/i.test(n))
        return true;
    if (/fuel tank|tank level|diesel level|diesel tank|bowser level|level \(l\)|level\(l\)/i.test(n)) {
        return true;
    }
    if (/\btank\b/i.test(n) && /\b(level|fuel|diesel|lit)/i.test(n))
        return true;
...
export function unitHasFuelLevelSensors(sens = []) {
...
export function unitHasFuelModuleSensors(sens = []) {
    return sens.some((s) => {
...
function defaultUnit(name, type, isFuel) {
    if (isFuel)
        return 'L';
    const n = name.toLowerCase();
    if (/temperature|\btemp\b/.test(n))
        return '°C';
    if (/battery|volt/.test(n))
        return 'V';
    return '';
}
...
export function readAllUnitSensors(item) {
    if (!item.sens)
        return [];
...
export function readFuelLevelSensors(item) {
...
export function totalLitersFromReadings(readings) {
    const fuel = readings.filter((r) => 'isFuelLevel' in r ? r.isFuelLevel : true);
    if (!fuel.length)
        return 0;
 ...
 */
const CAPACITY_NAME_RE = /tank[_\s-]?cap(?:acity)?|fuel[_\s-]?(?:tank[_\s-]?)?cap(?:acity)?|(?:tank|fuel|diesel|bowser|fls)[_\s-]?(?:size|vol(?:ume)?|max|full|capacity)|(?:capacity|volume|size|vol|max|full)\b.*\b(?:tank|fuel|diesel|bowser|fls)|(?:capacidad|capacidade|capacit[eé]|volumen|ёмкост\w*|tankvolumen|tankgröße)\b.*\b(?:tanque|tank|fuel|combustible|carburant|diesel|réservoir|deposito)|(?:tanque|tank|combustible|carburant)\b.*\b(?:capacidad|capacidade|capacit[eé]|volumen|ёмкост\w*)|^\s*(?:capacity|volume|tank\s*size|fuel\s*capacity|full\s*tank|capacidad|capacidade)\s*$/i;
/** Wialon cargo / body profile fields — never treat as fuel tank capacity. */
const CAPACITY_NAME_EXCLUDE = /carrying_capacity|effective_capacity|gross_vehicle_weight|engine_displacement|\bcargo\b|\bpayload\b/i;
function parsePositiveLitres(raw) {
...
function calibrationMaxLitres(tbl) {
 ...
 */
function capacitySlotKey(name) {
    return RESERVE_TANK_RE.test(name) ? 'reserve' : 'primary';
}
function sumDeclaredCapacity(fields) {
    const bySlot = new Map();
    for (const f of fields) {
        const name = f?.n || '';
        if (!name || CAPACITY_NAME_EXCLUDE.test(name))
            continue;
        if (!CAPACITY_NAME_RE.test(name))
            continue;
 ...
 */
function declaredTankCapacity(item) {
    const sources = [
...
function calibratedTankCapacity(item) {
    if (!item.sens)
        return undefined;
    let maxCap = 0;
    for (const sensor of Object.values(item.sens)) {
        if (!sensor?.n || !isFuelLevelSensor(sensor.n, sensor.t))
            continue;
 ...
 */
export function tankCapacityFromItem(item) {
 ...
 */
export function fuelPercentFromLitres(litres, capacity) {
    if (!(capacity > 0) || !Number.isFinite(litres) || litres < 0)
        return null;
    if (litres > capacity * 1.1)
        return null;
    return Math.min(100, Math.round((litres / capacity) * 100));
}
export function formatSensorSummary(readings) {
    if (!readings.length)
        return '';
    return readings.map((r) => `${r.name}: ${formatSensorValue(r.value, r.unit)}`).join(' · ');
}
export function formatFuelSensorSummary(readings) {
    if (!readings.length)
        return '';
