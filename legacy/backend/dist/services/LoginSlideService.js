...
export class LoginSlideService {
    static async ensureSchema() {
        ...
        `);
                ...
                console.warn('[login-slides] ensureSchema:', e.message);
            });
        }
        await schemaReady;
    }
    static async listAll() {
        ...
        const { rows } = await query(`SELECT id, title, details, eyebrow, image_url, mime_type, sort_order, is_enabled, created_at, updated_at
       FROM login_slides
    ...
    static async listPublic() {
        await this.ensureSchema();
        const { rows } = await query(`SELECT id, title, details, eyebrow, image_url, mime_type, sort_order, is_enabled
       FROM login_slides
       WHERE is_enabled = 1
       ORDER BY sort_order ASC, created_at ASC`);
        return rows.map(mapRow);
    ...
    static async get(id) {
        await this.ensureSchema();
        const { rows } = await query(`SELECT id, title, details, eyebrow, image_url, mime_type, sort_order, is_enabled, created_at, updated_at
       FROM login_slides WHERE id = $1 LIMIT 1`, [id]);
    ...
    static async create(input) {
        ...
        await query(`INSERT INTO login_slides
       ...
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`, [
        ...
        if (!row)
            throw new Error('Failed to create login slide');
        return row;
    }
    static async update(id, input) {
        ...
        if (!title)
            throw new Error('Title is required');
        await query(`UPDATE login_slides SET
         title = $1,
         details = $2,
         eyebrow = $3,
         image_url = $4,
       ...
       WHERE id = $9`, [
        ...
        const row = await this.get(id);
        if (!row)
            throw new Error('Slide not found after update');
        return row;
    }
    ...
    static async findImageByPublicUrl(publicUrl) {
        await this.ensureSchema();
        const { rows } = await query(`SELECT image_content, mime_type, image_url FROM login_slides WHERE image_url = $1 LIMIT 1`, [publicUrl]);
        const row = rows[0];
        if (!row)
            return null;
    ...
    static async nextSortOrder() {
        const { rows } = await query(`SELECT MAX(sort_order) AS m FROM login_slides`);
        return Number(rows[0]?.m ?? -1) + 1;
    }
    static async saveImage(slideId, fileName, mimeType, dataBase64) {
