...
export function classifyWialonAlertType(raw) {
    ...
    if (/harsh\s*corner|hard\s*turn|cornering|\bturn\b.*viol/.test(s))
        return 'harsh_cornering';
    if (/speeding|over\s*speed|speed\s*limit|max\s*speed|speed\s*viol/.test(s))
        return 'speeding';
    if (/\bidle\b|idling|excessive\s*idle/.test(s))
        return 'idling';
    if (/towing|tow\s*alert|unauthorized\s*move/.test(s))
        return 'towing';
    if (/\bsos\b|panic|alarm\s*button|emergency\s*button/.test(s))
        return 'sos';
    if (/eco\s*driv|drive\s*rank|reckless|driver\s*viol/.test(s))
        return 'eco_violation';
    ...
    if (/fuel\s*fill|filling|refuel|fuel\s*up|replenish|tank\s*fill/.test(s))
        return 'fuel_filling';
    ...
    if (/generat|genset|standby\s*power|backup\s*power/.test(s)) {
        ...
        if (/\b(off|stop|shutdown|stopped)\b/.test(s))
            return 'generator_off';
        if (/\b(on|start|started|running|run)\b/.test(s))
            return 'generator_on';
        return 'generator';
    }
    ...
    // Sensors & equipment
    if (/batter|voltage|pwr_ext|pwr_int|low\s*volt|external\s*power/.test(s))
        return 'battery';
    if (/temp(erature)?|overheat|coolant|thermostat/.test(s))
        return 'temperature';
    if (/door|hatch|cover\s*open|boot\s*open|trunk\s*open/.test(s))
        return 'door';
    if (/connect|offline|no\s*gps|gps\s*lost|signal\s*lost|link\s*down|no\s*signal|lost\s*connection/.test(s)) {
        return 'connection';
    }
    if (/maintain|service\s*interval|service\s*due|odometer\s*service/.test(s))
        return 'maintenance';
    if (/sensor|analog|digital\s*input|input\s*\d|param\s*alert|lls|counter/.test(s))
        return 'sensor';
    // Places / engine
    if (/geofence|geo\s*zone|zone\s*in|zone\s*out|geozone|route\s*control|enter\s*zone|leave\s*zone/.test(s)) {
        return 'geofence';
    }
    if (/ignition\s*on|engine\s*on|acc\s*on/.test(s))
        return 'ignition_on';
    if (/ignition\s*off|engine\s*off|acc\s*off/.test(s))
        return 'ignition_off';
...
export function severityForAlertType(type, flags) {
...
export function normalizeTaskMessages(raw) {
    if (!raw)
        return [];
    ...
    if (typeof raw === 'object') {
        ...
        if (o.messages && typeof o.messages === 'object') {
            return Object.values(o.messages);
        }
        // Some Wialon builds return the map at the top level.
        const vals = Object.values(o);
        if (vals.length && vals.every((v) => v && typeof v === 'object' && 't' in v)) {
            return vals;
        }
    }
    return [];
}
function classificationBlob(m, extras = []) {
    const p = m.p || {};
    return [
        ...extras,
        m.et,
...
export function mapTaskMessageToAlert(m, unitNameById) {
    return mapUnitMessageToAlert(m, unitNameById);
}
const NOISE_TITLE = /engine[\s_-]*hours?|mileage[\s_-]*(counter)?|odometer|counter[\s_-]*(reset|update|value)|initial[\s_-]*(mileage|engine)|gprs[\s_-]*traffic|traffic[\s_-]*counter|service[\s_-]*interval[\s_-]*hours|mh[\s_-]*counter|moto[\s_-]*hours?/i;
 ...
 */
export function isNoiseAlert(alert) {
    ...
    if (NOISE_TITLE.test(blob))
        return true;
    if (GENERIC_TITLE.test(bareTitle) && !desc)
        return true;
    if (!bareTitle && !desc)
        return true;
    return false;
