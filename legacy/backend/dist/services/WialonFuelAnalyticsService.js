...
import { applyBalanceConsumption, buildDailySummaries, buildLedgerFromTransactions, } from './wialonFuelLedger.js';
...
import { buildAnalytics, buildComparison, currentMonthKey, monthBounds, monthsInRange, previousMonthKeyFrom, resolvePeriod, } from './wialonFuelAnalytics.js';
...
const monthMemory = new Map();
const monthInflight = new Map();
function monthCacheKey(tenantId, yyyyMm) {
    return `${tenantId}:fuel:month:${CACHE_VERSION}:${yyyyMm}`;
}
function redisMonthKey(tenantId, yyyyMm) {
    return `fuel:month:${CACHE_VERSION}:${tenantId}:${yyyyMm}`;
}
function unitHasConsumption(list, unitId) {
...
export class WialonFuelAnalyticsService {
    ...
    static async loadCachedRowsOnly(tenantId, from, to) {
        ...
        await Promise.all(months.map(async (m) => {
            ...
            const redis = await cache.get(redisMonthKey(tenantId, m));
            if (redis?.length) {
    ...
    static isRangeWarming(tenantId, from, to) {
        for (const m of monthsInRange(from, to)) {
            if (monthInflight.has(`${monthCacheKey(tenantId, m)}:fetch`))
                return true;
        }
        return false;
    }
    /** Wait for background month cache (first load after restart). */
    static async waitForRangeCache(tenantId, from, to, maxMs = 200_000) {
        const months = monthsInRange(from, to);
        ...
        while (Date.now() < deadline) {
            ...
            if (rows.length > 0)
                return rows;
            if (!this.isRangeWarming(tenantId, from, to))
                break;
            await new Promise((r) => setTimeout(r, 2000));
        }
        return this.loadCachedRowsOnly(tenantId, from, to);
    }
    static async getAnalytics(tenantId, opts) {
        const period = opts.period ?? 'month';
        const resolved = resolvePeriod({ period, month: opts.month, from: opts.from, to: opts.to });
        const { rows, source, warmedMonths } = await this.loadRange(tenantId, resolved.from, resolved.to, opts.refresh ?? false);
        ...
        try {
            ...
            liveFuelByUnit = new Map(fleet.assets
                .filter((a) => {
                if (unitId != null && a.unitId !== unitId)
                    return false;
        ...
        const scopedRows = unitId != null ? rows.filter((r) => r.unitId === unitId) : rows;
        ...
        if (period !== 'month' || !resolved.month || !rows.length)
            return withLedger;
        ...
        const prevRows = unitId != null
            ? prevLoad.rows.filter((r) => r.unitId === unitId && r.timestamp >= prevFromTs && r.timestamp <= prevToTs)
        ...
        return {
            ...withLedger,
            comparison: buildComparison(withLedger.kpis, prevAnalytics.kpis, prevBounds.from, prevBounds.to, prevMonth),
        };
    }
    static async loadRange(tenantId, from, to, refresh) {
        const months = monthsInRange(from, to);
        const warmedMonths = [];
        const allRows = [];
        let anyWialon = false;
        let anyCache = false;
        const monthResults = await Promise.all(months.map((m) => this.ensureMonth(tenantId, m, refresh)));
    ...
    static async ensureMonth(tenantId, yyyyMm, refresh) {
        ...
        if (refresh) {
            ...
            const cache = new CacheService();
            const redis = await cache.get(redisMonthKey(tenantId, yyyyMm));
            if (redis?.length) {
                monthMemory.set(memKey, { rows: redis, expires: now + MONTH_TTL_MS });
    ...
    static async fetchAndCacheMonth(tenantId, yyyyMm) {
        const memKey = monthCacheKey(tenantId, yyyyMm);
    ...
    static async enrichMonthWithUnitReports(tenantId, yyyyMm, fromTs, toTs, initialRows, liveFuelByUnit) {
        const memKey = monthCacheKey(tenantId, yyyyMm);
        try {
