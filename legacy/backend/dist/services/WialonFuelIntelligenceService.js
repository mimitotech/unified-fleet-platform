...
export class WialonFuelIntelligenceService {
    static async getFuelIntelligence(tenantId, opts) {
        const [report, runtimeRows, fuelAssets] = await Promise.all([
            ...
            WialonFuelFleetService.listAssets(tenantId).catch(() => ({ assets: [] })),
        ]);
        const categoryByUnit = new Map();
        for (const a of fuelAssets.assets)
            categoryByUnit.set(a.unitId, a.assetType);
        const runtimeByUnit = new Map();
        const runtimeIntervalsByUnit = new Map();
        ...
        for (const tx of report.transactions) {
            ...
            const unitDaily = dailyByUnit.get(unitId) ?? new Map();
