...
function txToLedgerEntries(tx) {
    ...
    if (tx.section === 'filling') {
        ...
        if (vol <= 0 && tx.initialLevel <= 0 && tx.finalLevel <= 0)
            return [];
        return [
            {
                ...base,
    ...
    if (tx.section === 'consumption') {
        ...
        if (vol <= 0 && tx.mileage <= 0 && tx.durationSeconds <= 0)
            return [];
        return [
            {
                ...base,
...
function parseTripTimes(trip) {
    const fromBlock = trip.from;
    const toBlock = trip.to;
    const fromTs = Number(fromBlock?.t ?? trip.t1 ?? trip.tm ?? trip.time_begin ?? trip.begin ?? trip.from);
    const toTs = Number(toBlock?.t ?? trip.t2 ?? trip.time_end ?? trip.end ?? (typeof trip.to === 'number' ? trip.to : 0));
    return { fromTs, toTs };
}
/** Convert Wialon trip API rows into consumption ledger transactions. */
export function tripsToConsumptionTransactions(trips, unitId, unitName) {
    const out = [];
    for (const trip of trips) {
        ...
        if (!timestamp)
            continue;
        const fuelRaw = Number(trip.fuel ??
        ...
        const fuelBegin = Number(trip.fuel_begin ?? trip.fuelBegin ?? trip.fuel_level_begin ?? trip.fls_begin);
...
export function mergeTransactions(...groups) {
    const map = new Map();
...
export function buildLedgerFromTransactions(rows) {
    const entries = [];
 ...
 */
export function applyBalanceConsumption(rows, liveFuelByUnit) {
    ...
    for (const [unitId, unitRows] of byUnit) {
        ...
        const hasSummary = plausibleRows.some((r) => r.sensor === 'wialon_group_summary' &&
        ...
        if (closing <= 0) {
            for (let i = sorted.length - 1; i >= 0; i--) {
                const candidate = sorted[i].finalLevel > 0 ? sorted[i].finalLevel : sorted[i].initialLevel > 0 ? sorted[i].initialLevel : 0;
                if (candidate > 0 && isPlausibleBalanceLevel(candidate, live)) {
                    closing = candidate;
                    break;
                }
        ...
        if (!isPlausibleBalanceLevel(opening, live ?? closing))
            continue;
        const derived = opening > 0 || filled > 0 || closing > 0
        ...
        if (tankRef > 0 && derived > tankRef * 3 && derived > 500)
            continue;
...
export function buildDailySummaries(ledger, liveFuelByUnit) {
    const byKey = new Map();
    for (const e of ledger) {
        ...
        if (e.eventType === 'opening')
            row.openingFuel += e.balanceAfter ?? 0;
    ...
    const summaries = [...byKey.values()].sort((a, b) => a.date.localeCompare(b.date) || a.unitName.localeCompare(b.unitName));
    const byUnit = new Map();
    ...
    for (const [unitId, days] of byUnit) {
        let prevClosing = 0;
        for (const d of days) {
            if (d.openingFuel <= 0 && prevClosing > 0)
                d.openingFuel = round1(prevClosing);
