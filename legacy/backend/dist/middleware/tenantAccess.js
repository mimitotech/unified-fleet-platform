...
export async function canAccessTenant(userId, role, tenantId) {
    if (isSuperAdmin(role))
        return true;
    if (role === 'platform_admin') {
        const { rows } = await query(`SELECT 1 as ok FROM tenants WHERE id = $1 AND assigned_manager_id = $2`, [tenantId, userId]);
        return !!rows[0];
    }
    return false;
}
