...
export function computeFuelReportKpis(transactions: FuelTransaction[]): FuelReportKpis {
  let totalFilled = 0;
  let totalConsumed = 0;
  let totalMileage = 0;
  let theftEvents = 0;
...
export function computeVehicleFuelRows(transactions: FuelTransaction[]): VehicleFuelReportRow[] {
  ...
  for (const t of transactions) {
    ...
    if (t.section === 'consumption') {
      ...
      row.mileage += t.mileage || 0;
    }
    if (t.suddenFuelDrop > 0) {
      row.theftEvents += t.count || 1;
