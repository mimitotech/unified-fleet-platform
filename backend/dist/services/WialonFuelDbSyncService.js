...
function fuelTxToDbRow(tenantId, tx, assetCategory) {
...
export class WialonFuelDbSyncService {
    static async upsertTransactions(tenantId, txs, assetCategory) {
        if (!txs.length)
            return;
        ...
        for (const batch of batches) {
            ...
            for (const tx of batch) {
                ...
                params.push(row.id, row.tenantId, row.assetId, row.assetCategory, row.unitId, row.unitName, row.section, row.tank, row.timestamp, row.timeStr, row.location, row.latitude, row.longitude, row.initialLevel, row.finalLevel, row.filled, row.fuelUsed, row.mileage, row.avgConsumption, row.sensor, row.duration, row.durationSeconds, row.suddenFuelDrop, row.eventCount);
            }
            await query(`INSERT INTO fuel_transactions (${columns.join(', ')})
         VALUES ${values.join(', ')}
         ON CONFLICT (id) DO UPDATE
         SET ${updateSet}, updated_at = NOW()`, params);
        }
    }
