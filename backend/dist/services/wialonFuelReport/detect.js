...
function headerMatchesPattern(header, patterns) {
    const h = header.toLowerCase().trim();
    return patterns.some((p) => h.includes(p.toLowerCase()));
}
function headersMatchGroup(headers, requiredGroup) {
...
function findBestColumnIndex(headers, patterns, exclude = [], headerTypes) {
    let bestIdx = -1;
    let bestScore = 0;
    for (let i = 0; i < headers.length; i++) {
        const h = headers[i].toLowerCase().trim();
        if (exclude.some((e) => h.includes(e.toLowerCase())))
            continue;
        for (const p of patterns) {
            ...
            if (h === pl)
                score += 100;
            else if (h.startsWith(pl) || h.endsWith(pl))
                score += 25;
...
function detectTableSection(table) {
    ...
    if (WIALON_TABLE_SECTION[sysName])
        return WIALON_TABLE_SECTION[sysName];
    ...
    const order = ['filling', 'dispensed', 'theft', 'consumption'];
    for (const section of order) {
        const schema = FUEL_TABLE_SCHEMAS[section];
        if (!schema)
            continue;
        for (const requiredGroup of schema.requiredGroups) {
            if (headersMatchGroup(headers, requiredGroup))
                return section;
        }
    }
    return null;
}
function buildColumnMap(headers, section, headerTypes) {
    const schema = FUEL_TABLE_SCHEMAS[section];
    const columnMap = {};
    ...
    if (section === 'filling' && columnMap.filled < 0 && headerTypes?.length) {
        for (let i = 0; i < headers.length; i++) {
            const h = headers[i].toLowerCase();
            if (h.includes('initial') || h.includes('final') || h.includes('level') || h.includes('count'))
                continue;
            const t = Number(headerTypes[i]);
...
function detectTankType(tableName, tableLabel) {
    const combined = `${tableName} ${tableLabel}`.toLowerCase();
    if (/reserve|secondary|tank\s*2|tank2|aux(?:iliary)?|backup/.test(combined))
        return 'reserve';
    if (/main|primary|tank\s*1|tank1/.test(combined))
        return 'main';
    return 'main';
}
function isCombinedMultiTankTable(headers) {
    const normalizedHeaders = headers.map((h) => h.toLowerCase());
    let hasMain = false;
    let hasReserve = false;
    for (const header of normalizedHeaders) {
        if (TANK_HEADER_PATTERNS[0].patterns.some((p) => header.includes(p)))
            hasMain = true;
        if (TANK_HEADER_PATTERNS[1].patterns.some((p) => header.includes(p)))
            hasReserve = true;
    }
    return hasMain && hasReserve;
}
function buildTankColumnMaps(headers, _section) {
    const tankMaps = [];
    const normalizedHeaders = headers.map((h) => h.toLowerCase());
    for (const { tank, patterns } of TANK_HEADER_PATTERNS) {
        const tankMap = { tank, fuelUsed: -1, avgConsumption: -1, initialLevel: -1, finalLevel: -1 };
        for (let i = 0; i < normalizedHeaders.length; i++) {
            const header = normalizedHeaders[i];
            if (!patterns.some((p) => header.includes(p)))
                continue;
            if (header.includes('fuel used') || header.includes('consumed'))
                tankMap.fuelUsed = i;
            else if (header.includes('avg') || header.includes('km/l'))
                tankMap.avgConsumption = i;
        ...
        if (tankMap.fuelUsed !== -1 || tankMap.initialLevel !== -1 || tankMap.finalLevel !== -1)
            tankMaps.push(tankMap);
    }
    return tankMaps;
}
function isPerUnitPeriodSummaryTable(headers, section) {
    if (section !== 'consumption' && section !== 'filling' && section !== 'theft' && section !== 'dispensed') {
        return false;
    }
    const norm = headers.map((h) => h.toLowerCase().trim());
    const hasUnit = norm.some((h) => /^(unit|object|grouping|vehicle|generator|genset)$/.test(h) ||
        h.includes('unit name') ||
        h.includes('object name') ||
        h === 'name');
    const hasPeriod = (norm.some((h) => /beginning|begin|period start|^from$/.test(h)) &&
        norm.some((h) => /^end$|end time|period end|^to$|finish/.test(h))) ||
        norm.some((h) => /^time$|date/.test(h));
    const hasFuelMetric = norm.some((h) => /fuel used|fuel consumed|consumed|filled amount|fuel filled|filled|drained|sudden fuel|drain|final fuel|initial fuel|fuel level/.test(h));
    return hasUnit && hasPeriod && hasFuelMetric;
}
export function detectFuelTables(tables) {
    ...
    for (let tableIndex = 0; tableIndex < tables.length; tableIndex++) {
        ...
        detectedTables.push({
            ...
            isGroupUnitSummary: WIALON_GROUP_UNIT_SUMMARY.has(sysName) ||
