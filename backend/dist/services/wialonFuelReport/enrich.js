...
export function enrichTransactionsWithTankLevels(transactions) {
    ...
    for (const unitTransactions of groupedByUnit.values()) {
        ...
        for (const tx of sorted) {
            if (tx.tank === 'main' && tx.finalLevel > 0)
                lastMainLevel = tx.finalLevel;
            else if (tx.tank === 'reserve' && tx.finalLevel > 0)
                lastReserveLevel = tx.finalLevel;
            const sameTimestamp = sorted.filter((t) => t.timestamp === tx.timestamp);
            for (const t of sameTimestamp) {
                if (t.tank === 'main' && t.finalLevel > 0)
                    lastMainLevel = t.finalLevel;
                else if (t.tank === 'reserve' && t.finalLevel > 0)
                    lastReserveLevel = t.finalLevel;
            }
            enriched.push({
                ...tx,
