...
async function execFuelReport(client, template, objectId, fromTs, toTs) {
...
function dropRedundantStatsRows(transactions) {
    const unitsWithTripFuel = new Set();
    for (const r of transactions) {
        if (r.section === 'consumption' && r.sensor !== 'wialon_stats' && effectiveConsumed(r) > 0) {
            unitsWithTripFuel.add(r.unitId);
        }
    }
    return transactions.filter((r) => !(r.section === 'consumption' && r.sensor === 'wialon_stats' && unitsWithTripFuel.has(r.unitId)));
}
export async function processUnitFuelData(client, unit, template, fromTs, toTs) {
    const transactions = [];
    try {
...
export async function processGroupFuelData(client, group, template, fromTs, toTs, unitNameToId) {
    const transactions = [];
    ...
    try {
        ...
        for (const detected of detectedTables) {
            ...
            if (detected.isGroupUnitSummary && detected.section !== 'theft' && detected.rowCount > 0) {
                const topRows = await client.request('report/get_result_rows', {
                ...
                for (const row of tops) {
                    ...
                    const summary = processUnitGroupSummaryRow(cells, detected.columnMap, detected.headers, unit, toTs, detected.section, fromTs);
                    if (summary)
