...
function fuelSensorsFromItem(item) {
    if (!item.sens)
        return [];
    ...
    for (const [id, sensor] of Object.entries(item.sens)) {
        if (!sensor?.n || !sensor.p)
            continue;
        if (!isFuelLevelSensor(sensor.n, sensor.t))
            continue;
        const tank = RESERVE_RE.test(sensor.n)
            ? 'reserve'
...
function litersFromParams(params, sensors) {
    if (!params)
        return { main: null, reserve: null, total: null };
    let main = null;
    let reserve = null;
    if (sensors.length) {
    ...
    const total = main == null && reserve == null
        ? null
        : Math.round(((main ?? 0) + (reserve ?? 0)) * 10) / 10;
    return { main, reserve, total };
}
async function loadMessages(client, unitId, fromTs, toTs) {
    try {
...
function engineOnFromParams(params) {
    if (!params)
        return null;
    const keys = [
        'ignition',
    ...
    for (const [key, val] of Object.entries(params)) {
        if (!/ignition|engine.?on|genset|on.?off|gen.?status/i.test(key))
            continue;
...
function buildSeries(messages, sensors, fillThreshold, drainThreshold) {
    const draft = [];
    ...
    const processed = medianFilter(draft.map((d) => d.liters), win % 2 === 0 ? win + 1 : win);
    const out = [];
...
export class WialonFuelLevelSeriesService {
    static async getSeries(tenantId, input) {
        ...
        return withWialonClient(creds, async (client) => {
            ...
            try {
                const settings = (await client.request('unit/get_fuel_settings', {
                    itemId: unitId,
                }));
                const nested = (settings.fuelLevelParams || settings.fuel_level_params || settings);
                const fillings = (nested.fillings || settings.fillings);
                const thefts = (nested.thefts || settings.thefts);
