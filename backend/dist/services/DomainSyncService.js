...
function mapEcoSeverity(severity) {
...
async function resolveAssetId(tenantId, unitId) {
    if (!unitId)
        return null;
    const { rows } = await query(`SELECT am.asset_id
     ...
     LIMIT 1`, [tenantId, unitId]);
    return rows[0]?.asset_id ?? null;
}
export class DomainSyncService {
    /** Persist Wialon unit trips into trip_summaries (rolling window). */
    static async syncTenantTrips(tenantId) {
        const { rows: ds } = await query(`SELECT is_active, connection_verified_at, wialon_resource_id FROM data_sources
       WHERE tenant_id = $1 AND source_type = 'wialon' AND is_active = true`, [tenantId]);
        ...
        const { rows: cursor } = await query(`SELECT last_success_at FROM fuel_sync_cursor WHERE tenant_id = $1 AND cursor_key = $2`, [tenantId, cursorKey]);
        const last = cursor[0]?.last_success_at;
        if (last && Date.now() - new Date(last).getTime() < TRIP_SYNC_INTERVAL_MS)
            return 0;
        ...
        const parsed = await fetchTripsForUnits((id, f, t) => WialonLiveService.getUnitTrips(creds, id, f, t), units, from, to, 6);
        let upserted = 0;
        for (const row of parsed) {
            ...
            await query(`INSERT INTO trip_summaries (
       ...
       ON CONFLICT (tenant_id, cursor_key) DO UPDATE SET
         ...
         last_error = NULL`, [tenantId, cursorKey, now, upserted]);
        return upserted;
    }
    /** Persist eco / safety report rows into eco_driving_violations. */
    static async syncTenantEcoViolations(tenantId) {
        const { rows: ds } = await query(`SELECT is_active, connection_verified_at, wialon_resource_id FROM data_sources
       WHERE tenant_id = $1 AND source_type = 'wialon' AND is_active = true`, [tenantId]);
        if (!isWialonTenantConnected(ds[0]))
            return 0;
        ...
        const alerts = await withWialonClient(creds, async (client) => harvestEcoReportAlerts(creds, client, `eco-db:${tenantId}`, timeFrom, timeTo, unitIds, unitNameById));
        let upserted = 0;
        for (const alert of alerts) {
            ...
            const unitName = (unitId ? unitNameById.get(Number(unitId)) : undefined) ||
                unitId ||
                'Unknown';
            const assetId = unitId ? await resolveAssetId(tenantId, unitId) : null;
            const occurredAt = alert.timestamp instanceof Date ? alert.timestamp : new Date(alert.timestamp);
            await query(`INSERT INTO eco_driving_violations (
    ...
    static async syncAllConnectedTenants() {
