...
function cacheKey(tenantId, fromTs, toTs) {
    return `${tenantId}:${fromTs}:${toTs}`;
}
function parseDateRange(fromParam, toParam, days = 30) {
...
function isEngineHoursTable(headers) {
...
async function execReportTables(client, resourceId, templateId, groupId, fromTs, toTs) {
...
function rowId(unitId, beginning, end) {
    return createHash('sha256').update(`${unitId}|${beginning}|${end}`).digest('hex').slice(0, 16);
}
async function parseEngineHoursTable(client, tableIndex, rowCount, columnMap, groupName, unitNameToId) {
    const rows = [];
    ...
    for (const cells of leafCells) {
        ...
        if (engineHours === 0 && initialEngineHours === 0 && finalEngineHours === 0)
            continue;
        rows.push({
...
export class WialonGeneratorEngineHoursService {
    static async list(tenantId, opts) {
        ...
        const rows = await withWialonClient(creds, async (client) => {
            ...
            const group = groups.find((g) => /genset|generator/i.test(g.nm)) ?? groups[0] ?? null;
            if (!group)
                return [];
            const resolved = await WialonReportResolverService.findModuleTemplates(client, scope, 'engineHours', { includeFallback: true });
            ...
            if (groupTemplates.length) {
                for (const tmpl of groupTemplates) {
                    try {
                        const tables = await execReportTables(client, tmpl.resourceId, tmpl.templateId, group.id, fromTs, toTs);
                        for (let idx = 0; idx < tables.length; idx++) {
                            const table = tables[idx];
                            if (!isEngineHoursTable(table.header))
                                continue;
                            const columnMap = buildColumnMap(table.header);
                            const parsed = await parseEngineHoursTable(client, idx, table.rows, columnMap, group.nm, unitNameToId);
                ...
                for (const tmpl of unitTemplates) {
                    for (const unit of targets) {
                        try {
                            const tables = await execReportTables(client, tmpl.resourceId, tmpl.templateId, unit.id, fromTs, toTs);
                            for (let idx = 0; idx < tables.length; idx++) {
                                const table = tables[idx];
                                if (!isEngineHoursTable(table.header))
                                    continue;
                                const columnMap = buildColumnMap(table.header);
                                const parsed = await parseEngineHoursTable(client, idx, table.rows, columnMap, unit.nm, unitNameToId);
                                for (const row of parsed) {
                                    deduped.set(row.id, row);
                                }
                            }
        ...
        let out = rows;
        if (opts.unitId != null)
            out = out.filter((r) => r.unitId === opts.unitId);
