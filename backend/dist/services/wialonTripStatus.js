...
export function deriveStatusFromWialonEvents(item, events, opts) {
    if (!events)
    ...
    const minSpeed = item.rtd?.minMovingSpeed ?? 5;
    ...
    if (events.tripState === 2) {
        if (speed > minSpeed)
            return { status: 'moving', motionState: label || 'Stop' };
        if (events.ignitionOn === true)
            return { status: 'idle', motionState: label || 'Stop' };
        return { status: 'stopped', motionState: label || 'Stop' };
    }
    if (events.tripState === 0) {
        if (events.ignitionOn === true)
            return { status: 'idle', motionState: label || 'Parking' };
        if (events.ignitionOn === false)
            return { status: 'stopped', motionState: label || 'Parking' };
        if (speed > minSpeed)
            return { status: 'moving', motionState: label || 'Parking' };
        return { status: 'stopped', motionState: label || 'Parking' };
    }
    if (speed > minSpeed)
        return { status: 'moving', motionState: label };
    if (events.ignitionOn === true)
        return { status: 'idle', motionState: label };
    if (events.ignitionOn === false)
        return { status: 'stopped', motionState: label };
    return deriveWialonHostingStatus(item, undefined, opts);
}
