import { classifyWialonAlertType, isNoiseAlert, mapUnitMessageToAlert, normalizeTaskMessages, severityForAlertType, } from './wialonAlertClassify.js';
import { WialonLiveService } from './WialonLiveService.js';
import { logger } from '../config/logger.js';
const ecoReportCooldown = new Map();
const unitMsgCursor = new Map();
...
function exactRowFields(row, columns) {
...
function fieldValue(fields, patterns) {
    for (const field of fields) {
        if (patterns.some((p) => p.test(field.label)))
            return field.value.trim();
    }
    return undefined;
}
function rowTimestamp(fields, fallbackSec) {
...
function addAlert(byExternal, alert) {
    if (!alert.timestamp || Number.isNaN(alert.timestamp.getTime()))
        return;
    if (isNoiseAlert(alert))
        return;
    byExternal.set(alert.externalId || alert.id, alert);
}
async function safeUnload(client) {
    try {
...
async function loadUnitMessages(client, unitId, timeFrom, timeTo, flags, loadCount) {
    await safeUnload(client);
    try {
        const result = await client.request('messages/load_interval', {
            itemId: unitId,
            timeFrom,
            timeTo,
            flags,
...
export async function harvestTaskMessageAlerts(client, unitIds, unitNameById, timeFrom, timeTo) {
    ...
    for (let i = 0; i < unitIds.length; i += BATCH) {
        ...
        let gotAny = false;
        try {
            const result = await client.request('messages/get_task_messages', {
                itemIds: batch,
                timeFrom,
                timeTo,
                loadCount: 10000,
            ...
            for (const m of normalizeTaskMessages(result)) {
                if (!m?.t)
                    continue;
                if (m.item_id != null && !unitSet.has(m.item_id))
                    continue;
                gotAny = true;
        ...
        if (!gotAny) {
            for (const unitId of batch) {
                try {
                    const one = await client.request('messages/get_task_messages', {
                        itemId: unitId,
                        timeFrom,
                        timeTo,
                        loadCount: 500,
                    ...
                    for (const m of normalizeTaskMessages(one)) {
                        if (!m?.t)
                            continue;
                        addAlert(byExternal, mapUnitMessageToAlert({ ...m, item_id: m.item_id ?? unitId }, unitNameById, unitId));
                    }
                }
                catch {
                    /* skip */
 ...
 */
export async function harvestUnitEventAndNotificationAlerts(client, scopeKey, unitIds, unitNameById, timeFrom, timeTo) {
    if (!unitIds.length)
        return [];
    const byExternal = new Map();
 ...
 */
export async function harvestEcoReportAlerts(credentials, client, scopeKey, timeFrom, timeTo, allowedUnitIds, unitNameById) {
    if (!allowedUnitIds.length)
        return [];
    const last = ecoReportCooldown.get(scopeKey) ?? 0;
    if (Date.now() - last < ECO_COOLDOWN_MS)
        return [];
    ecoReportCooldown.set(scopeKey, Date.now());
    const allowedNames = new Set([...unitNameById.values()].map(normalizeUnitKey).filter(Boolean));
    try {
        const accountId = credentials.accountId;
        const resourceSpec = accountId !== undefined && accountId !== null && String(accountId).trim() !== ''
            ? {
        ...
        const resources = await client.request('core/search_items', {
            spec: resourceSpec,
            force: 1,
            flags: 8193,
            from: 0,
        ...
        for (const res of resources.items || []) {
            for (const [idStr, tmpl] of Object.entries(res.rep || {})) {
                const name = tmpl.n || '';
                if (!/eco\s*driv|violation|harsh|events?\s*report|safety/i.test(name))
                    continue;
                if (/\(group\)|\bgroup\b/i.test(name) && !/\(unit/i.test(name))
                    continue;
                resourceId = res.id;
            ...
            try {
                ...
                for (const table of report.tables) {
                    for (const row of table.rows) {
                        ...
                        const rowUnit = fieldValue(fields, [/^unit$/i, /^grouping$/i, /^object$/i, /^name$/i]) || unitName;
                        if (allowedNames.size && !allowedNames.has(normalizeUnitKey(rowUnit)))
                            continue;
                        const violation = fieldValue(fields, [
                            /^violation$/i,
                            /^criterion$/i,
                            /^criteria$/i,
                            /^event$/i,
                        ...
                        if (type === 'fleet_event' &&
                            !/brake|accel|speed|corner|idle|fuel|fill|theft|violation|eco|harsh|power|generat|sensor/i.test(classificationText)) {
                            continue;
                        }
                        const resolvedType = type === 'fleet_event'
