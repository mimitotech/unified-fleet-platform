...
function unitMatchKeys(unitName, plate) {
    const keys = new Set();
...
export class FuelVarianceService {
    static async getFlsTotalsByUnit(tenantId, fromDate, toDate) {
       ...
       GROUP BY unit_id, unit_name`, [tenantId, fromDate, toDate]);
        return rows.map((r) => ({
    ...
    static async getStationTotalsByKey(tenantId, fromDate, toDate) {
        const { rows } = await query(`SELECT
       ...
       GROUP BY registration_key`, [tenantId, fromDate, toDate]);
        const map = new Map();
        for (const r of rows) {
     ...
     */
    static async getStationLitersByUnitName(tenantId, fromDate, toDate) {
    ...
    static async getVarianceReport(tenantId, fromDate, toDate) {
        const stationByKey = await this.getStationTotalsByKey(tenantId, fromDate, toDate);
        const flsByKey = new Map();
        ...
        for (const t of flsTotals) {
            ...
            if (!keys.length)
                keys.push(normalizePlateKey(t.unitName) || t.unitName);
        ...
        for (const [unitName, fls] of flsByUnit) {
            ...
            if (stationLiters <= 0 && fls.liters <= 0)
                continue;
        ...
        const { rows: detailRows } = await query(`SELECT id, filled_at, registration, unit_name, product, quantity, unit_price, amount,
       ...
       LIMIT 2000`, [tenantId, fromDate, toDate]);
