...
function iconCacheKey(tokenHint, unitId, size, ugi) {
    return `wialon:icon:${tokenHint}:${unitId}:${size}:${ugi}`;
}
function tokenHint(creds) {
    const t = creds.token || '';
...
async function setCached(key, buf) {
    memory.set(key, { buf, expires: Date.now() + MEM_TTL_MS });
    pruneMemory();
    const redis = getRedis();
    if (!redis)
...
async function withPooledClient(creds, fn) {
...
setInterval(() => {
    ...
    for (const [key, slot] of sessions) {
        if (now - slot.lastUsed < SESSION_IDLE_MS)
            continue;
        sessions.delete(key);
...
export async function fetchCachedUnitIcon(creds, unitId, size = 32, ugi = 1) {
