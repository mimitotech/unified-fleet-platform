    ...
    try {
        ...
        for (const tenant of tenants) {
            try {
                ...
                await query(`UPDATE data_sources SET last_sync_at = NOW() WHERE tenant_id = $1 AND is_active = true`, [tenant.id]);
                assetCount++;
            }
            catch (err) {
        ...
        if (assetCount > 0)
            logger.info(`Synced assets for ${assetCount} tenants`);
        if (snapshotCount > 0)
            logger.info(`[FuelSync] stored live fuel snapshots for ${snapshotCount} tenants`);
    }
    catch (err) {
...
async function runFuelDbCycle() {
    ...
    try {
        ...
        if (count > 0)
            logger.info(`[FuelSync] synced fuel reports to database for ${count} tenants`);
    }
    catch (err) {
...
async function runDomainCycle() {
    ...
    try {
        ...
        if (count > 0)
            logger.info(`[DomainSync] synced trips/eco for ${count} Wialon tenants`);
    }
    catch (err) {
