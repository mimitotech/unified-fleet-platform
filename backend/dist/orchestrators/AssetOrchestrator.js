...
export class AssetOrchestrator {
    ...
    async initialize() {
        const { rows } = await query(`SELECT source_type, credentials_encrypted, is_active FROM data_sources WHERE tenant_id = $1 AND is_active = true`, [this.tenantId]);
        for (const row of rows) {
            try {
                const creds = await resolveSourceCredentials(this.tenantId, row.source_type, row.credentials_encrypted);
    ...
    async getUnifiedAssetsFromDb() {
        const { rows } = await query(`SELECT a.id, a.name, a.registration_plate, a.vin, a.make, a.model, a.year,
              am.source_type, am.external_id
       FROM assets a
       JOIN asset_mappings am ON am.asset_id = a.id
       WHERE a.tenant_id = $1
       ORDER BY a.name`, [this.tenantId]);
    ...
    async persistMappings(assets) {
        for (const asset of assets) {
           ...
           LIMIT 1`, [this.tenantId, src.type, src.id]);
                if (byMapping[0]) {
          ...
          ) LIMIT 1`, [this.tenantId, asset.vin || null, asset.registrationPlate || null]);
                assetId = byIdentity[0]?.id;
            }
            if (!assetId) {
                const ins = await query(`INSERT INTO assets (tenant_id, name, registration_plate, vin, make, model, year)
           VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id`, [this.tenantId, asset.name, asset.registrationPlate, asset.vin, asset.make, asset.model, asset.year]);
           ...
           year = COALESCE($7, year), updated_at = NOW() WHERE id = $1`, [assetId, asset.name, asset.registrationPlate, asset.vin, asset.make, asset.model, asset.year]);
            }
            for (const src of asset.sources) {
                await query(`INSERT INTO asset_mappings (asset_id, source_type, external_id)
           VALUES ($1, $2, $3) ON CONFLICT (asset_id, source_type) DO UPDATE SET external_id = $3`, [assetId, src.type, src.id]);
            }
        }
    }
    async getUnifiedStatus(assetId) {
        ...
        if (status)
            await this.cache.set(cacheKey, status, CACHE_STATUS_TTL);
        return status;
    }
    async fetchStatusForAsset(assetId) {
        const { rows: mappings } = await query(`SELECT source_type, external_id FROM asset_mappings am
       JOIN assets a ON a.id = am.asset_id
    ...
    scheduleBackgroundRefresh(cacheKey) {
        if (backgroundRefresh.has(this.tenantId))
            return;
        const job = (async () => {
            try {
                const results = await this.fetchAllStatusesFromAdapters();
    ...
    async getStoredStatusesFromDb() {
        const { rows } = await query(`SELECT a.id as asset_id, a.name, a.registration_plate,
    ...
    async persistStatusesToDb(items) {
        for (const item of items) {
            const st = item.status;
            const loc = st?.location;
            if (!st || !loc?.latitude || !loc?.longitude)
                continue;
            if (loc.latitude === 0 && loc.longitude === 0)
                continue;
            const source = (st.source || item.asset.sources[0]?.type);
            if (!source)
                continue;
    ...
    async fetchAllStatusesFromAdapters() {
        await this.initialize();
        const { rows: assetRows } = await query(`SELECT a.id as asset_id, a.name, a.registration_plate,
              am.source_type, am.external_id
       FROM assets a
       JOIN asset_mappings am ON am.asset_id = a.id
       WHERE a.tenant_id = $1`, [this.tenantId]);
        const byAsset = new Map();
        ...
        const wialonAdapter = this.adapters.get('wialon');
        ...
        const tracksolidAdapter = this.adapters.get('tracksolid');
        const tsBulk = new Map();
        ...
        const results = await Promise.all([...byAsset.values()].map(async (entry) => {
            const statuses = [];
