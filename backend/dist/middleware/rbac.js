...
export async function getTenantModuleAccess(tenantId, userId, role, moduleKey) {
    ...
    const { rows } = await query(`SELECT is_enabled, COALESCE(is_visible, true) as is_visible
     FROM tenant_modules WHERE tenant_id = $1 AND module_key = $2`, [tenantId, moduleKey]);
    const mod = rows[0];
    if (mod && !mod.is_enabled) {
        return { allowed: false, enabled: false, visible: false, canViewData: false };
    }
    const { rows: userMod } = await query(`SELECT is_enabled FROM user_modules WHERE user_id = $1 AND module_key = $2`, [userId, moduleKey]);
...
export function requireModule(moduleKey, options) {
    return async (req, res, next) => {
        ...
        if (req.user.tenantId) {
            const access = await getTenantModuleAccess(req.user.tenantId, req.user.id, req.user.role, moduleKey);
            if (!access.enabled) {
...
export function requireCommandAccess(req, res, next) {
    if (!req.user)
        return res.status(401).json({ success: false, error: 'Unauthorized' });
    if (COMMAND_ROLES.includes(req.user.role))
        return next();
