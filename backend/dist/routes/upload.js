...
router.post('/tenants/:id/upload', async (req, res, next) => {
    try {
        const { fileName, mimeType, data, fileType = 'logo' } = req.body;
        if (!fileName || !data)
            return error(res, 'fileName and data required');
        const base64 = data.includes(',') ? data.split(',')[1] : data;
        const buffer = Buffer.from(base64, 'base64');
        if (!buffer.length)
            return error(res, 'Empty file data');
        const mime = resolveUploadMime(fileName, mimeType);
        const result = await UploadService.saveTenantFile(String(req.params.id), fileType, fileName, mime, buffer);
        ...
        return success(res, {
            ...
            url: result.url,
            persisted: true,
            message: fileType === 'logo' || fileType === 'favicon'
