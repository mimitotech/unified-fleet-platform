...
import { FAILURE_SYSTEMS, sanitizeWorkshopAssetCategory, WORKSHOP_ASSET_CATEGORIES, } from '../../services/WorkshopChecklistTemplates.js';
const router = Router();
...
async function resolveWorkshopAssetId(tenantId, assetId, vehicleId) {
    const rawAsset = assetId != null ? String(assetId).trim() : '';
    if (rawAsset && UUID_RE.test(rawAsset))
        return rawAsset;
    const external = (vehicleId != null && /^\d+$/.test(String(vehicleId).trim()) ? String(vehicleId).trim() : '') ||
...
async function persistAssetCategory(assetId, category) {
    if (!assetId)
        return;
    try {
...
function legacyChecklistsFromSections(sections) {
...
router.get('/kpis', requireTenant, mod, async (req, res) => {
    const { rows } = await query(`WITH maint AS (
     ...
     SELECT
       ...
       ) AS fleet_health_score`, [req.tenantId]);
...
router.get('/inspections', requireTenant, mod, async (req, res) => {
    const { rows } = await query(`SELECT * FROM vehicle_inspections WHERE tenant_id = $1 AND deleted_at IS NULL ORDER BY inspection_date DESC LIMIT 100`, [req.tenantId]);
    return success(res, toCamelRows(rows));
});
router.get('/maintenance', requireTenant, mod, async (req, res) => {
    const { rows } = await query(`SELECT * FROM maintenance_logs WHERE tenant_id = $1 AND deleted_at IS NULL ORDER BY start_date DESC LIMIT 100`, [req.tenantId]);
    return success(res, toCamelRows(rows));
});
router.get('/breakdowns', requireTenant, mod, async (req, res) => {
    const { rows } = await query(`SELECT * FROM breakdown_reports WHERE tenant_id = $1 AND deleted_at IS NULL ORDER BY breakdown_time DESC LIMIT 100`, [req.tenantId]);
    return success(res, toCamelRows(rows));
});
router.get('/mechanics', requireTenant, mod, async (req, res) => {
    const { rows } = await query(`SELECT * FROM mechanics WHERE tenant_id = $1 AND deleted_at IS NULL AND is_active = true ORDER BY name`, [req.tenantId]);
    return success(res, toCamelRows(rows));
});
router.get('/checklist-templates', requireTenant, mod, async (req, res) => {
    ...
    const all = await Promise.all(WORKSHOP_ASSET_CATEGORIES.map(async (category) => {
        const tpl = await getChecklistTemplateForCategory(req.tenantId, category);
        return { ...tpl, failureSystems: FAILURE_SYSTEMS[category] };
    }));
    return success(res, all);
});
router.post('/inspections', requireTenant, mod, requireWriteAccess, async (req, res) => {
    const { vehicleId, vehicleName, vehiclePlate, assetId, driverId, driverName, inspectionType, inspectionDate, odometerReading, nextServiceMileage, truckHeadChecklist, trailerChecklist, overallStatus, notes, inspectorName, assetCategory, engineHours, checklistSections, } = req.body;
    if (!vehicleName || !inspectionType)
        return error(res, 'vehicleName and inspectionType required');
    ...
    const { rows } = await query(`INSERT INTO vehicle_inspections (
...
router.patch('/inspections/:id', requireTenant, mod, requireWriteAccess, async (req, res) => {
    const b = req.body;
    const resolvedAssetId = b.assetId != null || b.vehicleId != null
        ? await resolveWorkshopAssetId(req.tenantId, b.assetId, b.vehicleId)
        : undefined;
    const category = b.assetCategory != null ? sanitizeWorkshopAssetCategory(b.assetCategory) : null;
    if (resolvedAssetId && category)
        await persistAssetCategory(resolvedAssetId, category);
    const sectionsJson = b.checklistSections != null
        ? asJson(b.checklistSections, [])
        : null;
    const legacy = b.checklistSections != null ? legacyChecklistsFromSections(b.checklistSections) : null;
    const { rows } = await query(`UPDATE vehicle_inspections SET
    ...
    if (!rows[0])
        return error(res, 'Inspection not found', 404);
    return success(res, toCamelRows(rows)[0]);
});
router.delete('/inspections/:id', requireTenant, mod, requireWriteAccess, async (req, res) => {
    const { rowCount } = await query(`UPDATE vehicle_inspections SET deleted_at = NOW(), updated_at = NOW()
     WHERE id = $1 AND tenant_id = $2 AND deleted_at IS NULL`, [req.params.id, req.tenantId]);
    if (!rowCount)
        return error(res, 'Inspection not found', 404);
    return success(res, { deleted: true });
});
router.post('/maintenance', requireTenant, mod, requireWriteAccess, async (req, res) => {
    const { vehicleId, vehicleName, vehiclePlate, assetId, driverId, driverName, inspectionId, breakdownId, maintenanceType, priority, description, mechanicName, startDate, endDate, laborHours, laborCost, partsCost, totalCost, partsUsed, status, notes, odometerReading, nextServiceKm, nextServiceHours, nextServiceDays, assetCategory, engineHours, } = req.body;
    ...
    const category = sanitizeWorkshopAssetCategory(assetCategory);
    await persistAssetCategory(resolvedAssetId, category);
    const { rows } = await query(`INSERT INTO maintenance_logs (
       tenant_id, asset_id, vehicle_id, vehicle_name, vehicle_plate, driver_id, driver_name,
...
router.patch('/maintenance/:id', requireTenant, mod, requireWriteAccess, async (req, res) => {
    const b = req.body;
    const resolvedAssetId = b.assetId != null || b.vehicleId != null
        ? await resolveWorkshopAssetId(req.tenantId, b.assetId, b.vehicleId)
        : undefined;
    ...
    if (resolvedAssetId && category)
        await persistAssetCategory(resolvedAssetId, category);
    const { rows } = await query(`UPDATE maintenance_logs SET
       asset_id = COALESCE($3, asset_id),
       vehicle_id = COALESCE($4, vehicle_id),
       vehicle_name = COALESCE($5, vehicle_name),
       vehicle_plate = COALESCE($6, vehicle_plate),
    ...
    if (!rows[0])
        return error(res, 'Maintenance log not found', 404);
    return success(res, toCamelRows(rows)[0]);
});
router.delete('/maintenance/:id', requireTenant, mod, requireWriteAccess, async (req, res) => {
    const { rowCount } = await query(`UPDATE maintenance_logs SET deleted_at = NOW(), updated_at = NOW()
     WHERE id = $1 AND tenant_id = $2 AND deleted_at IS NULL`, [req.params.id, req.tenantId]);
    if (!rowCount)
        return error(res, 'Maintenance log not found', 404);
    return success(res, { deleted: true });
});
router.post('/breakdowns', requireTenant, mod, requireWriteAccess, async (req, res) => {
    const { vehicleId, vehicleName, vehiclePlate, assetId, driverId, driverName, location, breakdownTime, resolutionTime, severity, description, cause, resolution, downtimeHours, towingCost, repairCost, totalCost, tripId, assetCategory, failureSystem, } = req.body;
    if (!vehicleName || !description)
        return error(res, 'vehicleName and description required');
    const resolvedAssetId = await resolveWorkshopAssetId(req.tenantId, assetId, vehicleId);
    const category = sanitizeWorkshopAssetCategory(assetCategory);
    await persistAssetCategory(resolvedAssetId, category);
    const { rows } = await query(`INSERT INTO breakdown_reports (
       tenant_id, asset_id, vehicle_id, vehicle_name, vehicle_plate, driver_id, driver_name,
...
router.patch('/breakdowns/:id', requireTenant, mod, requireWriteAccess, async (req, res) => {
    const b = req.body;
    const resolvedAssetId = b.assetId != null || b.vehicleId != null
        ? await resolveWorkshopAssetId(req.tenantId, b.assetId, b.vehicleId)
        : undefined;
    ...
    if (resolvedAssetId && category)
        await persistAssetCategory(resolvedAssetId, category);
    const { rows } = await query(`UPDATE breakdown_reports SET
       asset_id = COALESCE($3, asset_id),
       vehicle_id = COALESCE($4, vehicle_id),
       vehicle_name = COALESCE($5, vehicle_name),
       vehicle_plate = COALESCE($6, vehicle_plate),
    ...
    if (!rows[0])
        return error(res, 'Breakdown report not found', 404);
    return success(res, toCamelRows(rows)[0]);
});
router.delete('/breakdowns/:id', requireTenant, mod, requireWriteAccess, async (req, res) => {
    const { rowCount } = await query(`UPDATE breakdown_reports SET deleted_at = NOW(), updated_at = NOW()
     WHERE id = $1 AND tenant_id = $2 AND deleted_at IS NULL`, [req.params.id, req.tenantId]);
    if (!rowCount)
        return error(res, 'Breakdown report not found', 404);
    return success(res, { deleted: true });
