...
const router = Router();
router.get('/data/:type', requireTenant, async (req, res) => {
    const type = req.params.type;
    const tenantId = req.tenantId;
    switch (type) {
        case 'trips': {
            const { rows } = await query(`SELECT * FROM trip_summaries WHERE tenant_id = $1 ORDER BY departure_time DESC LIMIT 500`, [tenantId]);
        ...
        case 'violations': {
            const { rows } = await query(`SELECT * FROM eco_driving_violations WHERE tenant_id = $1 ORDER BY occurred_at DESC LIMIT 500`, [tenantId]);
            return success(res, rows);
        }
        case 'drivers': {
            const { rows } = await query(`SELECT d.*, s.safety_score, s.fuel_efficiency, s.on_time_rate, s.violations_count
         FROM drivers d
         LEFT JOIN driver_performance_snapshots s ON s.driver_id = d.id AND s.snapshot_date = CURRENT_DATE
         WHERE d.tenant_id = $1 AND d.deleted_at IS NULL`, [tenantId]);
            return success(res, rows);
        }
        case 'workshop': {
            const { rows } = await query(`SELECT * FROM maintenance_logs WHERE tenant_id = $1 AND deleted_at IS NULL ORDER BY start_date DESC LIMIT 200`, [tenantId]);
            return success(res, rows);
        }
        case 'executive': {
...
router.get('/types', requireTenant, async (req, res) => {
    void req;
    return success(res, [
