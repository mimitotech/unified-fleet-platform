...
router.get('/transactions', requireTenant, async (req, res) => {
...
router.get('/kpis', requireTenant, async (req, res) => {
...
router.get('/monthly-trend', requireTenant, async (req, res) => {
    const { rows } = await query(`SELECT
     ...
     GROUP BY 1
     ORDER BY 1 ASC
     LIMIT 12`, [req.tenantId]);
    return success(res, toCamelRows(rows));
});
/** Latest stored sensor snapshots (historical). Live tank levels still come from Wialon. */
router.get('/live-snapshots', requireTenant, async (req, res) => {
    const rows = await WialonFuelLiveSnapshotService.getLatestByTenant(req.tenantId);
    return success(res, toCamelRows(rows));
});
router.get('/sync-status', requireTenant, async (req, res) => {
    const { rows } = await query(`SELECT cursor_key, last_synced_at, last_success_at, row_count, last_error
     FROM fuel_sync_cursor
     WHERE tenant_id = $1
     ORDER BY last_synced_at DESC`, [req.tenantId]);
    return success(res, toCamelRows(rows));
});
router.post('/sync', requireTenant, async (req, res) => {
...
router.get('/variance', requireTenant, async (req, res) => {
