import type { WialonFleetSnapshot, WialonFleetUnit } from './WialonFleetService.js';
import { extractFuelLevel } from './wialonFuel.js';

export { extractFuelLevel } from './wialonFuel.js';

export function fleetUnitToAsset(u: WialonFleetUnit) {
  return {
    id: String(u.id),
    name: u.name,
    registrationPlate: u.plate,
    sources: [{ type: 'wialon' as const, id: String(u.id) }],
    hwName: u.hwName,
    hw: u.hw,
  };
}

export function fleetUnitToStatusItem(u: WialonFleetUnit) {
  ...
  return {
    ...
    status: u.position
      ? {
          ...
          source: 'wialon' as const,
        }
      : { status: u.status, fuelLevel, fuelFormatted, fuelLiters, source: 'wialon' as const },
    wialon: {
      wialonId: u.id,
      hwName: u.hwName,
      hw: u.hw,
...
export function fleetToSnapshotResponse(fleet: WialonFleetSnapshot, live = true) {
  return {
    live,
    stale: false,
    ...fleet,
