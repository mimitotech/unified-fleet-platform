import type { AssetStatus } from '@ufp/shared';
import { query } from '../config/database.js';
import type { WialonSearchItem } from '../adapters/wialonUtils.js';

export function wialonUnitPlate(unit: WialonSearchItem): string | undefined {
  return unit.prp?.registration_plate || unit.prp?.plate || undefined;
}

export function wialonUnitVin(unit: WialonSearchItem): string | undefined {
  return unit.prp?.vin || undefined;
}

export function statusFromWialonPos(pos?: WialonSearchItem['pos']): AssetStatus | null {
  if (!pos) return null;
  if (pos.y === 0 && pos.x === 0) return null;
...
export async function upsertWialonUnit(
  tenantId: string,
  unit: WialonSearchItem
): Promise<string> {
  ...
  if (!assetId && (vin || registrationPlate)) {
    const { rows: byIdentity } = await query<{ id: string }>(
      ...
      [tenantId, vin || null, registrationPlate || null]
    );
  ...
  if (!assetId) {
    const ins = await query<{ id: string }>(
      ...
      [tenantId, name, registrationPlate || null, vin || null, make || null, model || null, year || null]
    );
    ...
    await query(
      ...
      [assetId, name, registrationPlate, vin, make, model, year]
    );
  }
