...
type FuelReading = { t: number; liters: number };

function round1(n: number) {
  return Math.round(n * 10) / 10;
}

function parseFuelFromMessage(msg: Record<string, unknown>): number | null {
  const p = msg.p as Record<string, unknown> | undefined;
  if (!p) return null;
...
async function loadFuelReadings(
  ...
  try {
    ...
    while (indexFrom < total) {
      ...
      const batch = await client.request<{ messages?: Array<Record<string, unknown>> }>(
      ...
      for (const msg of batch.messages ?? []) {
        ...
        if (Number.isFinite(t) && liters != null) readings.push({ t, liters });
      }
      indexFrom = indexTo + 1;
    }
...
function consumptionFromReadings(
  ...
  const byDay = new Map<string, FuelReading[]>();
  ...
  for (const [day, dayReadings] of byDay) {
    ...
    for (let i = 1; i < sorted.length; i++) {
      ...
      if (diff >= minRefillJump) filled += diff;
      else if (diff < -0.3) consumedFromDrops += -diff;
    }
...
export async function supplementTransactionsWithMessages(
  tenantId: string,
  ...
  if (unitIds?.length) {
    ...
    for (const r of rows) {
      if (r.unitId > 0) targets.set(r.unitId, r.unitName);
    }
  }
