...
export class WialonFuelReportCapabilityService {
  static async getFuelCapabilities(tenantId: string) {
    ...
    return withWialonClient(creds, async (client) => {
      ...
      const modules: WialonReportModule[] = ['fuel', 'engineHours', 'trips', 'events', 'emissions'];
      const capabilities: FuelModuleCapability[] = modules.map((module) => {
