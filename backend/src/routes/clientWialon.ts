...
import {
  getTenantWialonRow,
  loadTenantWialonCreds,
} from '../services/tenantWialonCredentials.js';
...
const live = createWialonLiveHandlers(async (req: TenantRequest) =>
  loadTenantWialonCreds(req.tenantId!)
);
const crud = createWialonCrudHandlers(async (req: TenantRequest) =>
  loadTenantWialonCreds(req.tenantId!)
);
...
async function requireWialonCreds(tenantId: string) {
  return loadTenantWialonCreds(tenantId);
}

/** Safe Wialon context for any tenant user (no token, no full user list). */
router.get('/wialon/context', requireTenant, async (req: TenantRequest, res) => {
  ...
  const meta = (row.wialon_session_meta || {}) as Record<string, unknown>;
  const rawCounts = meta.counts as Record<string, number> | undefined;
  const scopedAccountId = meta.scopedAccountId as number | undefined;
  const counts =
  ...
  let motherAccountName: string | null = null;
  ...
  return success(res, {
    ...
    sessionUserName: meta.sessionUserName ?? (meta.sessionUser as { nm?: string } | undefined)?.nm,
    sessionMeta: meta,
    counts,
    lastSyncAt: row.last_sync_at,
    lastError: wialonSyncWarning(row),
    syncWarning: (meta.syncWarning as string | undefined) || null,
...
router.post('/wialon/sync', requireTenant, async (req: TenantRequest, res) => {
  if (!isTenantAdmin(req)) return error(res, 'Forbidden', 403);
  try {
    ...
    await query(
      `UPDATE data_sources SET last_error = $2, updated_at = NOW()
       WHERE tenant_id = $1 AND source_type = 'wialon'`,
      [req.tenantId!, (e as Error).message]
    );
    return error(res, (e as Error).message);
  }
});

router.get('/wialon/capabilities', requireTenant, async (req: TenantRequest, res) => {
  try {
...
router.get('/wialon/routes', requireTenant, async (req: TenantRequest, res) => {
  try {
    const creds = await requireWialonCreds(req.tenantId!);
...
router.get('/wialon/reports/templates', requireTenant, async (req: TenantRequest, res) => {
  try {
    const creds = await requireWialonCreds(req.tenantId!);
...
router.get('/wialon/accounts', requireTenant, async (req: TenantRequest, res) => {
  if (!isTenantAdmin(req)) return error(res, 'Forbidden', 403);
  try {
    const creds = await requireWialonCreds(req.tenantId!);
...
router.get('/wialon/fleet', requireTenant, async (req: TenantRequest, res) => {
  try {
...
router.get('/wialon/units/:unitId/fuel/settings', requireTenant, async (req: TenantRequest, res) => {
  try {
...
router.get('/wialon/units/:unitId/fuel/live', requireTenant, async (req: TenantRequest, res) => {
  try {
    const unitId = Number(req.params.unitId);
    const creds = await requireWialonCreds(req.tenantId!);
...
router.get('/wialon/fuel/analytics', requireTenant, async (req: TenantRequest, res) => {
  try {
    ...
    const data = await WialonFuelAnalyticsService.getAnalytics(req.tenantId!, {
      unitId: unitId ?? null,
      unitName,
      period: period as 'day' | 'week' | 'month' | 'year' | 'custom',
      month,
      from,
      to,
      refresh,
...
router.get('/wialon/fuel/transactions', requireTenant, async (req: TenantRequest, res) => {
  try {
    const refresh = req.query.refresh === 'true';
    req.setTimeout(20 * 60 * 1000);
...
router.get('/wialon/fuel/live', requireTenant, async (req: TenantRequest, res) => {
  try {
...
router.get('/wialon/fuel/assets', requireTenant, async (req: TenantRequest, res) => {
  try {
...
router.get('/wialon/fuel/dashboard', requireTenant, async (req: TenantRequest, res) => {
  try {
    const data = await WialonFuelFleetService.listAssets(req.tenantId!);
    return success(res, data);
  } catch (e) {
...
router.get('/wialon/fuel/overview', requireTenant, async (req: TenantRequest, res) => {
  try {
    const from = req.query.from ? String(req.query.from) : undefined;
    const to = req.query.to ? String(req.query.to) : undefined;
    const refresh = req.query.refresh === 'true';
...
router.get('/wialon/fuel/events', requireTenant, async (req: TenantRequest, res) => {
  try {
...
router.get('/wialon/fuel/generator-engine-hours', requireTenant, async (req: TenantRequest, res) => {
  try {
    req.setTimeout(20 * 60 * 1000);
    const from = req.query.from ? String(req.query.from) : req.query.startDate ? String(req.query.startDate) : undefined;
    const to = req.query.to ? String(req.query.to) : req.query.endDate ? String(req.query.endDate) : undefined;
...
router.get('/wialon/units/:unitId/fuel/profile', requireTenant, async (req: TenantRequest, res) => {
  try {
    const unitId = Number(req.params.unitId);
...
router.patch('/wialon/units/:unitId/fuel/math', requireTenant, requireCommandAccess, async (req: TenantRequest, res) => {
  try {
    const unitId = Number(req.params.unitId);
    const { idling, urban, suburban } = req.body as { idling: number; urban: number; suburban: number };
    const creds = await requireWialonCreds(req.tenantId!);
...
router.patch('/wialon/units/:unitId/fuel/rates', requireTenant, requireCommandAccess, async (req: TenantRequest, res) => {
  try {
    const unitId = Number(req.params.unitId);
    const creds = await requireWialonCreds(req.tenantId!);
...
router.patch('/wialon/units/:unitId/fuel/detection', requireTenant, requireCommandAccess, async (req: TenantRequest, res) => {
  try {
    const unitId = Number(req.params.unitId);
    const creds = await requireWialonCreds(req.tenantId!);
...
router.post('/wialon/units/:unitId/fuel/sensors', requireTenant, requireCommandAccess, async (req: TenantRequest, res) => {
  try {
    const unitId = Number(req.params.unitId);
    const creds = await requireWialonCreds(req.tenantId!);
    const { name, parameter, calibration, description } = req.body as {
      name: string;
      parameter: string;
...
router.post('/wialon/units/:unitId/fuel/events/mark-false', requireTenant, requireCommandAccess, async (req: TenantRequest, res) => {
  try {
    const unitId = Number(req.params.unitId);
    const creds = await requireWialonCreds(req.tenantId!);
    const { resourceId, eventType, timeFrom, timeTo, eventId } = req.body as {
      resourceId: number;
...
router.post('/wialon/units/:unitId/fuel/filling', requireTenant, requireCommandAccess, async (req: TenantRequest, res) => {
  try {
    const unitId = Number(req.params.unitId);
    const creds = await requireWialonCreds(req.tenantId!);
...
router.get('/wialon/reports/live/fleet-status', requireTenant, async (req: TenantRequest, res) => {
  try {
...
router.get('/wialon/reports/live/fleet-fuel', requireTenant, async (req: TenantRequest, res) => {
  try {
...
router.get('/wialon/reports/live/trips', requireTenant, async (req: TenantRequest, res) => {
  try {
...
router.get('/wialon/reports/live/unit-sensors/:unitId', requireTenant, async (req: TenantRequest, res) => {
  try {
...
router.post('/wialon/api', requireTenant, async (req: TenantRequest, res) => {
  const svc = String(req.body?.svc || '');
  const params = (req.body?.params || {}) as Record<string, unknown>;
  if (!svc) return error(res, 'svc is required', 400);
  try {
    const creds = await requireWialonCreds(req.tenantId!);
