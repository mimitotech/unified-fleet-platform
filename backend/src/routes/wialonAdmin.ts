...
router.param('id', (req, res, next, id) => {
  if (!UUID_RE.test(id)) return error(res, 'Invalid id: must be a valid UUID', 400);
  return next();
});
...
router.get('/tenants/:id/wialon/accounts/:accountId/units', async (req, res) => {
  try {
    ...
    if (Number.isNaN(accountId)) return error(res, 'Invalid Wialon account id');
...
router.post('/tenants/:id/wialon/link-account', async (req: AuthRequest, res) => {
  try {
    const accountId = parseInt(String(req.body?.accountId ?? ''), 10);
    if (Number.isNaN(accountId)) return error(res, 'accountId is required');
    ...
    const selectedUserIds = Array.isArray(req.body?.wialonUserIds)
      ? (req.body.wialonUserIds as unknown[]).map((id) => parseInt(String(id), 10)).filter((n) => !Number.isNaN(n))
      : undefined;
    const motherAccountId = req.body?.motherAccountId ? String(req.body.motherAccountId) : undefined;
    const result = await WialonAccountLinkService.linkAccount(
...
router.get('/wialon/overview', async (_req, res) => {
  try {
    const { rows } = await query<{
      ...
      wialon_resource_id: string | null;
      wialon_operate_as: string | null;
      is_active: boolean;
    ...
    return success(res, {
      tenants: rows.map((r) => ({
        ...
        accountTier: (r.wialon_session_meta as { accountTier?: string } | null)?.accountTier,
        counts: (r.wialon_session_meta as { counts?: Record<string, number> } | null)?.counts,
