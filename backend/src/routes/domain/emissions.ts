...
router.get('/violations', requireTenant, async (req: TenantRequest, res) => {
  ...
  const { rows } = await query(
    `SELECT * FROM eco_driving_violations WHERE tenant_id = $1 ORDER BY occurred_at DESC LIMIT $2`,
    [req.tenantId, limit]
  );
  return success(res, toCamelRows(rows));
});

router.get('/metrics', requireTenant, async (req: TenantRequest, res) => {
  const { rows: trips } = await query(
    `SELECT COALESCE(SUM(mileage), 0)::float as mileage, COALESCE(SUM(fuel_used), 0)::float as fuel_used
     FROM trip_summaries WHERE tenant_id = $1`,
    [req.tenantId]
  );
  const { rows: fuel } = await query(
    `SELECT COALESCE(SUM(fuel_used), 0)::float as fuel_used, COALESCE(SUM(mileage), 0)::float as mileage
     FROM fuel_transactions WHERE tenant_id = $1 AND section = 'consumption'`,
    [req.tenantId]
  );
  const { rows: violations } = await query(
    `SELECT COUNT(*)::int as count FROM eco_driving_violations WHERE tenant_id = $1`,
    [req.tenantId]
  );

  const totalFuel = (trips[0]?.fuel_used as number || 0) + (fuel[0]?.fuel_used as number || 0);
  const totalMileage = Math.max(trips[0]?.mileage as number || 0, fuel[0]?.mileage as number || 0);
...
router.get('/by-vehicle', requireTenant, async (req: TenantRequest, res) => {
  const { rows } = await query(
    `SELECT unit_name, COALESCE(SUM(fuel_used), 0)::float as fuel_used, COALESCE(SUM(mileage), 0)::float as mileage
     FROM trip_summaries WHERE tenant_id = $1
     GROUP BY unit_name ORDER BY fuel_used DESC`,
    [req.tenantId]
  );
