...
export class PlatformIntegrationService {
  static async getStatus(sourceType: SourceType): Promise<{
    configured: boolean;
    connected: boolean;
    ...
    const { rows } = await query<PlatformIntegrationRow>(
      `SELECT source_type, is_active, connection_verified_at, last_error, session_meta
       FROM platform_integrations WHERE source_type = $1`,
      [sourceType]
    );
    const row = rows[0];
    if (!row) {
  ...
  static async loadWialonCreds(motherId?: string): Promise<WialonCredentialsInput> {
  ...
  static async loadWialonCredsLegacy(): Promise<WialonCredentialsInput> {
    const { rows } = await query<{ credentials_encrypted: string }>(
      `SELECT credentials_encrypted FROM platform_integrations
       WHERE source_type = 'wialon' AND is_active = true`,
      []
    );
    if (!rows[0]) throw new Error('Wialon Center is not configured. Add the mother account token in Admin → Wialon Center.');
    const stored = decryptCredentials(rows[0].credentials_encrypted) as Record<string, unknown>;
    const token = String(stored.token || '').trim();
    if (!token) throw new Error('Wialon Center token is missing. Re-save the mother account in Wialon Center.');
    const creds: WialonCredentialsInput = { token };
    if (stored.baseUrl) creds.baseUrl = String(stored.baseUrl).trim();
    return creds;
  }

  static async isWialonConfigured(): Promise<boolean> {
    const mothers = await WialonMotherAccountService.list();
    if (mothers.some((m) => m.isActive)) return true;
    const { rows } = await query<{ n: number }>(
      `SELECT COUNT(*)::int AS n FROM platform_integrations WHERE source_type = 'wialon' AND is_active = true`,
      []
    );
    return (rows[0]?.n || 0) > 0;
  }

  static async saveWialon(credentials: Record<string, unknown>): Promise<{
    meta: Record<string, unknown>;
    counts: Awaited<ReturnType<typeof WialonHierarchyService.probe>>['counts'];
    accountTier: string;
    ...
    const counts = (meta.counts || {}) as Awaited<ReturnType<typeof WialonHierarchyService.probe>>['counts'];
    ...
    await query(
      `INSERT INTO platform_integrations (source_type, credentials_encrypted, is_active, connection_verified_at, last_error, session_meta, updated_at)
  ...
  static async saveWialonLegacy(credentials: Record<string, unknown>): Promise<{
    meta: Record<string, unknown>;
    counts: Awaited<ReturnType<typeof WialonHierarchyService.probe>>['counts'];
    accountTier: string;
  }> {
  ...
  static async getWialonAccountAssignments(): Promise<
  ...
  static async assertAccountAvailable(accountId: number, exceptTenantId?: string): Promise<void> {
    const { rows } = await query<{ tenant_id: string; tenant_name: string }>(
      `SELECT ds.tenant_id, t.name AS tenant_name
       FROM data_sources ds
       INNER JOIN tenants t ON t.id = ds.tenant_id
