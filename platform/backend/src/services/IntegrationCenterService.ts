...
export type IntegrationTenantRow = {
  tenantId: string;
  tenantName: string;
  tenantSlug: string;
  isActive: boolean;
...
export class IntegrationCenterService {
  static async getCenterStatus(sourceType: Extract<SourceType, 'loconav' | 'tracksolid'>) {
    ...
    const tenants: IntegrationTenantRow[] = rows.map((r) => ({
