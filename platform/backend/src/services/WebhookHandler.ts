...
const LOCONAV_KIND_MAP: Record<string, { type: string; title: string; severity: 'info' | 'warning' | 'critical' }> = {
...
export class WebhookHandler {
  ...
  static async handleLocoNav(tenantId: string, body: Record<string, unknown>): Promise<void> {
    ...
    const meta = LOCONAV_KIND_MAP[kind] || { type: kind, title: `LocoNav: ${kind}`, severity: 'warning' as const };
...
function buildLocoNavDescription(body: Record<string, unknown>, kind: string): string {
  const parts: string[] = [];
  if (body.rpm_value) parts.push(`RPM: ${body.rpm_value}`);
  if (body.vehicle_number) parts.push(`Vehicle: ${body.vehicle_number}`);
  if (body.start_time) parts.push(`Start: ${body.start_time}`);
  if (body.inactive_event_time) parts.push(`End: ${body.inactive_event_time}`);
...
async function resolveAssetId(tenantId: string, externalId: unknown): Promise<string | undefined> {
  ...
  const { rows } = await query<{ asset_id: string }>(
    `SELECT am.asset_id FROM asset_mappings am
     ...
     WHERE a.tenant_id = $1 AND (am.external_id = $2 OR a.name ILIKE $2 OR a.registration_plate ILIKE $2) LIMIT 1`,
