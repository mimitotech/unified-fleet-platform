...
export class TrackSolidSyncService {
  static async syncTenant(tenantId: string): Promise<{ vehicles: number; geofences: number; alerts: number }> {
    const { rows: ds } = await query<{ credentials_encrypted: string }>(
      `SELECT credentials_encrypted FROM data_sources WHERE tenant_id = $1 AND source_type = 'tracksolid' AND is_active = true`,
      [tenantId]
    );
    if (!ds[0]) {
    ...
    try {
      ...
      for (const f of fences) {
        ...
        if (existing[0]) {
          await query(
            `UPDATE geofences SET geometry = $2, updated_at = NOW() WHERE id = $1`,
            [existing[0].id, geometry]
          );
        } else {
          await query(
            `INSERT INTO geofences (tenant_id, name, type, geometry, color, is_active)
             VALUES ($1, $2, $3, $4, $5, true)`,
            [tenantId, name, fenceType, geometry, String(f.fence_color || '#004225')]
          );
        }
        geofencesSynced++;
      }
    ...
    try {
      ...
      for (const alert of alerts) {
        const { rows: existing } = await query<{ id: string }>(
          `SELECT id FROM alerts WHERE tenant_id = $1 AND source_type = 'tracksolid' AND external_id = $2 LIMIT 1`,
          [tenantId, alert.externalId]
        );
        if (existing[0]) continue;
        await query(
          `INSERT INTO alerts (tenant_id, source_type, external_id, type, severity, title, description,
