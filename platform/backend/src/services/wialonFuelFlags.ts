...
export const CALC_TYPE_FLAGS: Array<{ bit: number; label: string }> = [
...
export const FUEL_LEVEL_PARAM_FLAGS: Array<{ bit: number; label: string }> = [
...
export function decodeBitFlags(value: number | undefined, defs: Array<{ bit: number; label: string }>): string[] {
  if (value == null || !Number.isFinite(value)) return [];
  return defs.filter((d) => (value & d.bit) !== 0).map((d) => d.label);
}

export type FuelLevelParamsInput = {
  flags?: number;
...
export function normalizeFuelLevelParams(raw?: Record<string, unknown>): FuelLevelParamsInput {
  if (!raw) return {};
  return {
    ...
    fillingsJoinInterval:
      raw.fillingsJoinInterval != null ? Number(raw.fillingsJoinInterval) : Number(raw.fillings_join_interval) || undefined,
    theftsJoinInterval:
      raw.theftsJoinInterval != null ? Number(raw.theftsJoinInterval) : Number(raw.thefts_join_interval) || undefined,
    extraFillingTimeout:
      raw.extraFillingTimeout != null ? Number(raw.extraFillingTimeout) : Number(raw.extra_filling_timeout) || undefined,
  };
}
