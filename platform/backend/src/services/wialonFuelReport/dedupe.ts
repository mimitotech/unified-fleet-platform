...
function contentKey(row: FuelTransaction): string {
  return [
    row.unitId,
    row.section,
    row.tank ?? 'main',
...
function preferRow(a: FuelTransaction, b: FuelTransaction): FuelTransaction {
...
export function dedupeFuelTransactions(rows: FuelTransaction[]): FuelTransaction[] {
  ...
  const byContent = new Map<string, FuelTransaction>();
