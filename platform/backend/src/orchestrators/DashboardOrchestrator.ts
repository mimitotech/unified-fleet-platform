...
async function safeQuery<T extends Record<string, unknown> = Record<string, unknown>>(
  ...
  try {
    ...
    logger.warn(`Dashboard KPI query failed (${label}): ${(err as Error).message}`);
    return [];
  }
}
...
export class DashboardOrchestrator {
  ...
  async getKpis() {
    ...
    const [alertRows, driverStats, routeStats, fuelStats, workshopStats] = await Promise.all([
      ...
      safeQuery<{ total: number; active: number }>(
      ...
      safeQuery<{ active_routes: number; scheduled_routes: number }>(
      ...
      safeQuery<{ fuel_consumed_month: number }>(
      ...
      safeQuery<{ pending_maintenance: number }>(
